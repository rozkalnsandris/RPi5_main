...
TARGET_NEW_BLOB = "ca3e94935c9871d85eb4d8d13ab470ad3ad66b78"
TARGET_OLD_SHA256 = "48c8c5fb0cdc005bf0e4fbb05a203297e05d7ef62689ddd0d6d717c13acc0fcb"
TARGET_NEW_SHA256_BINDING = "DERIVED_FROM_EXACT_TARGET_BLOB_PRE_MUTATION"
UPGRADE_CONTRACT_RELATIVE = Path("ops/deploy/weather-public-runtime-operator-upgrade-v10.json")
CHECKOUT_CONTRACT_RELATIVE = Path("ops/deploy/rpi5-main-weather-public-runtime-operator-upgrade-v10-trusted-checkout-bootstrap.json")
UPGRADE_ENTRYPOINT_RELATIVE = Path("ops/bin/rozkalns-weather-public-runtime-operator-upgrade-v10")
UPGRADE_MODULE_RELATIVE = Path("ops/lib/deploy_executor/weather_public_runtime_operator_upgrade_v10.py")
TEMP_NAME = ".weather_public_runtime_operator.py.compatibility-upgrade-v10.tmp"
...
def _target_source_bytes() -> bytes:
    path = _repository_root() / TARGET_SOURCE
    try:
        meta = path.lstat()
        if (
            ...
            or meta.st_nlink != 1
            or meta.st_size < 1
            or meta.st_size > _engine.MAX_ARTIFACT_BYTES
        ):
            raise WeatherOperatorUpgradeError("v10 target source identity drifted")
        return path.read_bytes()
    except WeatherOperatorUpgradeError:
        raise
    except OSError as exc:
        raise WeatherOperatorUpgradeError("v10 target source is unavailable") from exc


TARGET_NEW_SHA256 = hashlib.sha256(_target_source_bytes()).hexdigest()


def source_readiness() -> dict[str, Any]:
    return {
        "schema": "rozkalns-weather.public-runtime-operator-upgrade-v10-source.v1",
        "issue": ISSUE,
        "caller_authority": (),
        ...
        "target_new_sha256_binding": TARGET_NEW_SHA256_BINDING,
...
def _validate_upgrade_contract(value: dict[str, Any]) -> None:
    expected = {
        "schema": "rozkalns-weather.public-runtime-operator-upgrade-v10.v1",
        ...
        "new_sha256_binding": TARGET_NEW_SHA256_BINDING,
    ...
    for key, wanted in expected.items():
        if value.get(key) != wanted:
            raise WeatherOperatorUpgradeError(f"operator upgrade v10 contract drifted: {key}")
    if value.get("caller_arguments") != [] or value.get("allowed_changed_artifacts") != [TARGET_SOURCE]:
        raise WeatherOperatorUpgradeError("operator upgrade v10 caller/artifact contract drifted")
    if value.get("replacement") != {
        ...
        raise WeatherOperatorUpgradeError("operator upgrade v10 replacement contract drifted")
    if value.get("safety") != {
        ...
        raise WeatherOperatorUpgradeError("operator upgrade v10 safety contract drifted")
...
def _derive_checkout() -> Path:
    ...
    if checkout.name != TRUSTED_CHECKOUT_NAME or Path(__file__).resolve() != expected.resolve():
        raise WeatherOperatorUpgradeError("Weather operator upgrade v10 is not in the fixed trusted upgrade checkout")
    return checkout


def _receipt(*, result: str, source_sha: str | None, state: dict[str, bool], reason: str | None = None) -> dict[str, Any]:
    value: dict[str, Any] = {
        "schema": "rozkalns-weather.public-runtime-operator-upgrade-v10-receipt.v1",
        "result": result,
