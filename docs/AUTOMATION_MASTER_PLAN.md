...
- [ ] Reconcile the CV #152 control-plane baseline through RPi5_main issue #163 before any `AUTO_DEPLOY_SAFE` execution canary: first separately authorize/install/prove only the exact classifier blob from current CV main while production remains on `f5431265232f356fa27f6204f0cba56e1e730928`; then separately authorize the existing manual production-canary boundary to reconcile production to exact current CV main `4a0069a97022841da07a687a197ea8cfacc56cd6`. Keep the recurring timer disabled/inactive and legacy runner unchanged throughout both gates.
...
- RPi5_main issue #163 now tracks the required #152 control-plane baseline reconciliation before any AUTO canary. Source-only host-alignment gate PR #164 merged as `73879f4821791d938349e9f79f44ce1a0bd55ab0`; it pins old classifier blob `e9020c00328122a1a028c9734002f0ea1c956f2f`, target classifier blob `7fb09d469eaeb574b2bba39474cc7a6bb55504da`, exact CV target `4a0069a97022841da07a687a197ea8cfacc56cd6`, rollback-on-proof-failure, timer/service/legacy/pull-transport invariants and post-install `MANUAL_ROLLOUT_REQUIRED`/`CONTROL_PLANE_CHANGED=true`/mutation=false. PR-head Validate run `31482148993` PASS; merge does not authorize the classifier host mutation or later manual production alignment, and fresh exact-main push CI remains required immediately before host mutation;
...
The #144 rollback/pre-mutation and health-recovery gate is complete. CV production is reconciled to `f5431265232f356fa27f6204f0cba56e1e730928`, transactional public checks passed, readiness is `CURRENT`, and the recurring CV pull timer remains disabled/inactive. The #151 classifier runtime-contract audit is also complete at current CV main `4a0069a97022841da07a687a197ea8cfacc56cd6`, but that merge changed the `runner/` control plane after the last production reconciliation. Therefore the first incomplete Phase 3 gate is RPi5_main issue #163: align/prove the exact new classifier on the host while production remains at `f543126...`, then separately reconcile production through the existing manual production-canary boundary to exact current CV main `4a0069a9...`. Only after both control-plane and production baselines are reconciled may a genuinely newer independent `AUTO_DEPLOY_SAFE` delta become eligible for the one-shot controller execution canary.
...
**Phase 3 only:** the first incomplete gate is RPi5_main issue #163, created because CV PR #152 changed the deploy-impact classifier control plane after production had already been reconciled to `f5431265232f356fa27f6204f0cba56e1e730928`. Current exact CV main is `4a0069a97022841da07a687a197ea8cfacc56cd6`. Do not prepare or run an `AUTO_DEPLOY_SAFE` controller canary until this baseline mismatch is removed.

First, after fresh exact-current-main push CI and exact source/host identity revalidation, separately authorize the reviewed #163 classifier host-alignment operator. It may replace only the installed classifier with exact blob `7fb09d469eaeb574b2bba39474cc7a6bb55504da`, must preserve production at `f5431265232f356fa27f6204f0cba56e1e730928`, must leave preflight/pull-library/pull-wrapper/legacy helper and timer/service states unchanged, and must require post-install App-authenticated `MANUAL_ROLLOUT_REQUIRED`, `CONTROL_PLANE_CHANGED=true`, exact target/production/CI and `PRODUCTION_MUTATION_AUTHORIZED=false`. A failure after installation must restore the prior classifier blob.

Second, only after that host proof is green, separately review and authorize the existing explicit manual production-canary boundary to reconcile production to exact current CV main `4a0069a97022841da07a687a197ea8cfacc56cd6`. This is a distinct production mutation authorization and must retain exact-target approval, installed-artifact identity, timer-OFF, transactional rollback/public-contract checks, post-deploy readiness `CURRENT`, and unchanged legacy helper/runner invariants.

Only after both classifier and production baselines are reconciled to `4a0069a9...` may the program wait for a genuinely newer exact-current-main CV delta that independently classifies `AUTO_DEPLOY_SAFE`. That later one-shot controller canary must still require fresh exact-current-main push CI, `CONTROL_PLANE_CHANGED=false`, exact installed preflight/classifier/pull-library/pull-wrapper identities, the exact controller↔wrapper evidence contract, identical `READY`/`AUTO_DEPLOY_SAFE` preflight results immediately before mutation, transactional public MIME/cache/nosniff/CSP evidence, final production at the exact target SHA, post-deploy readiness `CURRENT`, timer still disabled/inactive, and unchanged legacy helper/runner. Do not enable recurring execution as part of the canary.

CV issue #151 is complete and reconciled by PR #152; the broad `bot/` automatic runtime allowance is no longer a pending recurring-execution blocker. Recurring CV timer activation is still forbidden until #163 classifier/production baseline reconciliation and the later genuine one-shot `AUTO_DEPLOY_SAFE` execution canary are green. Legacy self-hosted runner retirement remains the final Phase 3 gate and requires its own review/authorization after recurring replacement execution evidence is green.
