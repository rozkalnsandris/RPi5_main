...
SCRIPT_RELATIVE = "scripts/install-deploy-executor-p9-executor-auth-upgrade.py"
...
TARGET = TargetSpec(
    "ops/lib/deploy_executor/p9_runtime.py",
    Path("/usr/local/lib/rozkalns-deploy-executor/deploy_executor/p9_runtime.py"),
    "977621d420337f5f627fcfc90da5e41e9cd8e739",
    0o644,
)
...
def _preflight(expected_sha: str) -> bytes:
    ...
    if os.geteuid() != 0:
        raise UpgradeError("P9 executor-auth upgrade requires root")
...
def _write_fd_all(fd: int, data: bytes) -> None:
    ...
    while written < len(view):
        ...
        if count <= 0:
            raise UpgradeError("short write while replacing reviewed P9 runtime")
        written += count
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "P9 executor-auth runtime module. Source merge alone never authorizes --apply."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.apply:
            print(
                "P9_EXECUTOR_AUTH_UPGRADE_PREFLIGHT=PASS "
                f"source_sha={args.expected_sha}"
            )
            print("P9_EXECUTOR_AUTH_MUTATION=NO")
            return 0
        ...
        print(f"P9 executor-auth upgrade refused: {exc}", file=sys.stderr)
        return 1

    print(
        "P9_EXECUTOR_AUTH_UPGRADE=PASS "
        f"source_sha={args.expected_sha}"
    )
    print("TARGETS_REPLACED=1")
    print("NETWORK_REQUEST=NO")
    print("CREDENTIAL_READ=NO")
    print("GITHUB_TOKEN_MINT=NO")
    print("D1_REQUEST=NO")
