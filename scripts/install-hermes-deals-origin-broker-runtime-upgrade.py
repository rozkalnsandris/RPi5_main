...
SCRIPT_RELATIVE = 'scripts/install-hermes-deals-origin-broker-runtime-upgrade.py'
...
RECEIPT_SCHEMA = 'rozkalns.hermes-deals.origin-broker-runtime-upgrade-receipt.v1'
...
REPLACE_TARGETS = (
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_privileged_broker.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_privileged_broker.py'), '2543278ee48f184a79ac67c70e7f77c06cfbd7c8', '7656fe3c0ba7cfe1865ec000ab77503569beb7bd', 0o644),
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_helper_launch.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_helper_launch.py'), '5f190ebdcfdbc2a12242843733cb9740202cc9bd', '4e29a13202cfb49003d9922e5ceeb905bb00a774', 0o644),
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_broker_composition.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_broker_composition.py'), 'a7a9421527fb5b2ed0f250446dc257f0a9ac8a29', '571f7f51cf10013c66343c1c8c27ddfaf8cd92c0', 0o644),
    TargetSpec('ops/bin/rozkalns-hermes-deals-origin-broker', Path('/usr/local/libexec/rozkalns-hermes-deals-origin-broker'), '211b968b0c8ef6a0a7d73ce50a53d6bac7d2cc2f', '49aced1a1e6dd0c986947f53f06b23eba590aec4', 0o755),
    TargetSpec('ops/systemd/rozkalns-hermes-deals-origin-broker@.service', Path('/etc/systemd/system/rozkalns-hermes-deals-origin-broker@.service'), '2a304e70550f17092b9cafd365bbf6d05d23893b', '21319746d1e32f2b67f701f0a22174bfb0542987', 0o644),
)

CREATE_TARGETS = (
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_adapter.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_adapter.py'), None, '0b20cefa0c9193ecb8e7811c855767d56baf59ce', 0o644),
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_dispatch_request.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_dispatch_request.py'), None, 'bd58fa89a6eade662cf1e942d48ef5381c4e679b', 0o644),
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_privileged_consumer.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_privileged_consumer.py'), None, 'af0987064710afedd02cd56f40b9e3a3cacfaf00', 0o644),
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_privileged_dispatcher.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_privileged_dispatcher.py'), None, 'ae51af60bf2094be944d13c303bf6a94c0117d69', 0o644),
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_broker_runtime.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_broker_runtime.py'), None, '53112450cbccbabd9a5ec62966b9fd96d3513bea', 0o644),
    TargetSpec('ops/lib/deploy_executor/hermes_deals_origin_runtime_adapters.py', Path('/usr/local/lib/rozkalns-deploy-executor/deploy_executor/hermes_deals_origin_runtime_adapters.py'), None, '456fea3d6969975d0fd432d20089772f28b63ec7', 0o644),
)
...
def _require_exact_source(expected_sha: str) -> None:
    ...
    if tracked != Path(__file__).read_bytes():
        _fail('upgrade operator bytes differ from exact expected source SHA')
...
def _require_replace_prestate(target: TargetSpec) -> None:
    ...
    temp = target.target_path.parent / ('.' + target.target_path.name + '.hermes-runtime-upgrade.tmp')
    if temp.exists() or temp.is_symlink():
        _fail(f'fixed upgrade temporary target already exists: {temp}')
...
def _preflight(expected_sha: str) -> dict[str, bytes]:
    if os.geteuid() != ROOT_UID:
        _fail('broker runtime upgrade preflight requires root read context')
...
def _systemctl_mutation(state: dict[str, object], *args: str) -> None:
    if tuple(args) not in SYSTEMCTL_MUTATIONS:
        _fail('systemctl mutation is outside the fixed upgrade allowlist')
...
def _replace_target(target: TargetSpec, data: bytes, state: dict[str, object]) -> None:
    ...
    temp_name = '.' + target.target_path.name + '.hermes-runtime-upgrade.tmp'
...
def _receipt(result: str, source_sha: str, state: dict[str, object], reason: str | None=None) -> str:
    value={
        ...
        'replay_write_path':str(REPLAY_STATE_DIR),'global_registry_mutation':False,
...
def apply(expected_sha: str, state: dict[str, object]) -> str:
    ...
    return _receipt('HERMES_ORIGIN_BROKER_RUNTIME_UPGRADE_PASS', expected_sha, state)


def preflight(expected_sha: str, state: dict[str, object]) -> str:
    _preflight(expected_sha)
    return _receipt('HERMES_ORIGIN_BROKER_RUNTIME_UPGRADE_PREFLIGHT_READY', expected_sha, state)


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description='Fail-closed exact Hermes broker runtime upgrade operator; source merge never authorizes --apply.'
    )
