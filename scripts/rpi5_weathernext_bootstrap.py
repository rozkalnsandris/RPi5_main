...
MANAGER_CHECKOUT = "/var/lib/rpi5-deploy/RPi5_main"
...
QUEUE_FIELDS = frozenset(
    {
        "source_repository", "exact_git_sha_or_waiting_merge",
        "source_pr_or_issue_if_applicable", "target_alias", "execution_location_class",
        "repository_entrypoint", "expected_baseline_when_observable", "read_only_preflight",
        "verification_and_reconciliation", "allowed_mutation_categories_and_limits",
        "explicit_exclusions", "dependencies_if_any", "deploy_class_and_extra_owner_gate_requirement",
    }
)
...
def capability_descriptor() -> dict[str, str]:
    return {
        "status": "HOST_V12_WEATHERNEXT_BOOTSTRAP_CAPABILITY_INSTALLED",
...
def _gh_api_json(path: str) -> tuple[Mapping[str, Any], datetime]:
    ...
    separators = [match.start() for match in re.finditer(r"\n\n(?=[\[{])", raw)]
    if not separators:
...
def verify_authorization_unchanged(
    ...
    *,
    ...
    if (
        ...
        or current.request_id != accepted.request_id
        or current.canonical_payload_sha256 != accepted.canonical_payload_sha256
...
def _git_checkout(path: pathlib.Path, *args: str, check: bool = True) -> str:
    result = run(
...
def _manager_checkout() -> pathlib.Path:
    manager = CTX.rooted(MANAGER_CHECKOUT)
    info = manager.lstat()
    ...
    if _git_checkout(manager, "remote", "get-url", "origin") != REVIEWED_ORIGIN:
...
def _manager_snapshot(manager: pathlib.Path) -> tuple[str, str]:
    return (
        _git_checkout(manager, "rev-parse", "HEAD"),
        _git_checkout(manager, "status", "--porcelain=v1", "--untracked-files=all"),
    )
...
def _remote_main_sha(manager: pathlib.Path) -> str:
    output = _git_checkout(manager, "ls-remote", "--exit-code", "origin", "refs/heads/main")
...
def _trusted_checkout_state(manager: pathlib.Path, source_sha: str) -> str:
    ...
    reviewed_bytes = _git_checkout(manager, "show", f"{source_sha}:{ENTRYPOINT_SOURCE}").encode("utf-8")
    ...
    if (
        ...
        or destination.read_bytes() != reviewed_bytes
    ):
...
def _fetch_source(manager: pathlib.Path, source_sha: str) -> None:
    ...
    if _git_checkout(manager, "rev-parse", "refs/remotes/origin/main^{commit}") != source_sha:
...
def _reviewed_source_bytes(manager: pathlib.Path, trusted: pathlib.Path, source_sha: str) -> bytes:
    ...
    reviewed = _git_checkout(manager, "show", f"{source_sha}:{ENTRYPOINT_SOURCE}").encode("utf-8")
...
def execute_weathernext_bootstrap(authorization_issue_number: int) -> dict[str, Any]:
    ...
    with operation_lock():
        ...
        try:
            ...
            try:
                append_log(
                    f"WEATHERNEXT_BOOTSTRAP STOP consumed=1 stage={stage} retry=NO cleanup=NO rollback=NO"
                )
            except Exception:
                pass
            raise BootstrapStop(
