...
The #144 rollback/pre-mutation and health-recovery gate is complete. CV production is reconciled to exact current `f5431265232f356fa27f6204f0cba56e1e730928`, transactional public checks passed, readiness is `CURRENT`, and the recurring CV pull timer remains disabled/inactive. Phase 3 may now advance only to the reviewed #140 controller/readiness host installation and one-shot execution canary gate; recurring timer activation is still not authorized.
...
**Phase 3 only:** install and host-prove the reviewed #140 CV controller/readiness execution artifacts from exact current `RPi5_main/main` while keeping `rozkalns-cv-pull-deploy.timer` disabled/inactive and preserving the legacy self-hosted release path. Revalidate exact RPi5_main source/CI, exact installed CV preflight/classifier/pull-library/pull-wrapper identities, current CV production/main reconciliation at `f5431265232f356fa27f6204f0cba56e1e730928`, and the #141 maintenance-policy boundary before any one-shot execution test.

After the #140 artifact installation gate is green, prepare a separately reviewed `AUTO_DEPLOY_SAFE` CV canary target and require fresh exact-current-main push CI plus `CONTROL_PLANE_CHANGED=false`. Run exactly one one-shot controller execution with the recurring timer still disabled; require the controller to cross the root-owned pull-helper boundary only after identical preflight results immediately before mutation, then require transactional public MIME/cache/nosniff/CSP evidence, final production at the exact target SHA, post-deploy readiness `CURRENT`, legacy helper/runner unchanged, and timer still disabled/inactive.
