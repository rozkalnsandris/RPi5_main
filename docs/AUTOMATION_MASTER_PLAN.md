...
- [x] Fix the CVBot build-input identity so every current Dockerfile `COPY` input is covered; CV PR #150 merged as `f5431265232f356fa27f6204f0cba56e1e730928` with full PR-head CI run `31373969253` PASS. Exact-main push CI remains a mandatory revalidation immediately before recovery mutation.
- [ ] Recover CV through the existing project-owned explicit manual canary/pull deploy to exact current `rozkalns-cv/main=f5431265232f356fa27f6204f0cba56e1e730928`; require fresh exact-main push CI, exact installed pull-artifact identities, `MANUAL_ROLLOUT_REQUIRED`, `CONTROL_PLANE_CHANGED=false`, transactional rollback/public checks, healthy `cvbot` `/health/ready`, local/public CV PASS, production-state reconciliation, and the CV pull timer disabled/inactive. Keep reopened CV #144 open until this healthy recovery evidence is green.
...
- [ ] Enable the recurring local controller only after the rollback regression gate, live-health recovery prerequisite and one-shot `AUTO_DEPLOY_SAFE` execution canary are green.
- [ ] Disable/remove the public-repo self-hosted release runner only after the replacement execution path, rollback regression gate, live-health recovery prerequisite and public verification are green.
...
- current recovery snapshot from tracker: production `8afead7cae3f36882abca1b519af5bdc138978ce`, CV main `f5431265232f356fa27f6204f0cba56e1e730928`; delta class `MANUAL_ROLLOUT_REQUIRED`, `CONTROL_PLANE_CHANGED=false`. Exact-main push CI and installed pull-artifact identities remain fail-closed checks immediately before mutation; CV #144 is currently reopened until healthy recovery evidence is green;
...
The #144 rollback/pre-mutation code path remains regression-proven, but issue #144 is currently reopened as the healthy-recovery evidence gate. The #140 source boundary must not be installed or exercised on the host until the explicit manual recovery to exact current CV main `f5431265232f356fa27f6204f0cba56e1e730928` is complete with `cvbot` healthy and the CV pull timer still disabled/inactive.
...
**Phase 3 only:** prepare the explicit manual CV recovery to exact current `rozkalns-cv/main=f5431265232f356fa27f6204f0cba56e1e730928`. Before any production mutation, revalidate through the RPi5 GitHub App that this SHA is still exact current `main` and has successful exact-main push CI; revalidate the installed pull library/wrapper identities, current production state, `MANUAL_ROLLOUT_REQUIRED`, `CONTROL_PLANE_CHANGED=false`, legacy helper/runner invariants, and `rozkalns-cv-pull-deploy.timer` disabled/inactive. This plan reconciliation does not authorize the recovery mutation itself.

After those preconditions are green, require separate explicit approval for one transactional project-owned manual canary/pull deploy to `f5431265232f356fa27f6204f0cba56e1e730928`. Success must prove final production at that exact SHA, rollback-capable public MIME/cache/nosniff/CSP checks, `cvbot` `/health/ready` HTTP 200 and Docker `healthy`, local/public CV checks PASS, readiness/state reconciliation, timer still disabled/inactive, and the legacy self-hosted path unchanged. Record sanitized evidence under CV #144 and RPi5_main #103; only then may the healthy-recovery gate be closed.

Only after that recovery gate is green may Phase 3 install the reviewed #140 controller/readiness artifacts and run the separately authorized one-shot `AUTO_DEPLOY_SAFE` execution canary. Only after that one-shot canary is green may recurring CV pull timer activation be reviewed and separately authorized. Legacy self-hosted runner retirement remains the final Phase 3 gate and requires its own review/authorization after recurring replacement execution evidence is green.