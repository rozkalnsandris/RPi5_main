...
1. validate the backup filename and verify the exact sidecar SHA-256 before
   decryption;
2. verify the RPi5 operator and Hermes verifier are clean files from the exact
   requested Git SHAs;
3. require a non-production work directory and a conservative free-space
   preflight;
4. create a fresh `0700` workspace and decrypt to a temporary `0600` tarball;
5. validate the whole archive, then re-check free space against the declared
...
6. require the V12 metadata manifest plus the complete Hermes restore shape;
7. extract **only** `home/andris/hermes-tech` into the isolated restore root;
8. invoke the reviewed Hermes `tools/verify_restore_root.py` from the exact
   source SHA;
9. retain only a bounded sanitized verifier summary;
10. remove the whole plaintext workspace on success or failure;
11. write the final evidence only after cleanup has been attempted. A cleanup
...
It must not contain the age identity path or bytes, `.env` values, article/DB
rows, retained archive plaintext, visitor data, remote-storage identifiers,
full restore paths, tokens or credentials.

## Example command shape

The values below are placeholders. Resolve exact current SHAs and the selected
retained archive at execution time; do not copy historical SHAs from an issue or
chat.
...
SHAs, leave `/home/andris/hermes-tech` and service/DB state unchanged, record
RPO/RTO evidence, and prove cleanup. Only that canary can satisfy the remaining
host evidence for `hermes-tech#90`.
