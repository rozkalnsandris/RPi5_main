...
def test_exact_operator_integration_is_required() -> None:
    payload = load_raw()
    payload["approved_operator_integrations"][0]["app_id"] += 1
    expect_error(payload, "operator app id drifted")
    payload = load_raw()
    payload["approved_operator_integrations"].append(
        deepcopy(payload["approved_operator_integrations"][0])
    )
    expect_error(payload, "exactly one approved operator integration")
...
def test_bound_id_alone_does_not_enable_runtime() -> None:
    payload = load_raw()
    payload["authorization_repository_id"] = 123456789
    contract = isolated.validate_contract(payload)
    assert contract.authorization_repository_id == 123456789
    assert contract.activation_enabled is False
    assert contract.runtime_binding_ready is False
