...
grep -Fq 'PRE_MUTATION_REVALIDATION=PASS' "$operator"
grep -Fq 'GitHub main moved during activation preflight' "$operator"
grep -Fq 'working-tree source bytes changed' "$operator"
...
grep -Fq 'systemctl daemon-reload || ok=false' "$operator"
grep -Fq '[[ -n "$(unit_fragment "$SERVICE_UNIT")" || -n "$(unit_fragment "$TIMER_UNIT")" ]]' "$operator"

pre_line="$(grep -nF 'PRE_MUTATION_REVALIDATION=PASS' "$operator" | cut -d: -f1)"
