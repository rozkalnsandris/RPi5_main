...
- [ ] **P10 PREPARATION / STRICT PREFLIGHT WAIT:** `ops-workflows#28` is the selected lowest-risk genuine ordinary candidate, binding `dashboard_RPi5@2ef105d191f7e1a97dd219745c7d6d29aabbb5ea` to `dashboard-rpi5-production-release`. It is `WAITING_STRICT_READ_ONLY_PRODUCTION_PLAN_BASELINE`, not READY. Fresh source and exact-source CI remain green; production release identity, installed-controller provenance, candidate digest, target-release state and apply-lock state remain trusted-host runtime facts. The latest trusted-host attempt stopped before any preparation mutation because the expected local Dashboard source checkout was absent; no fetch, worktree, root PLAN, apply or production mutation occurred. After this source reconciliation is merged and exact-main CI is green, obtain a fresh explicit STRICT P10 PREFLIGHT owner authorization; resolve/prepare one exact-source candidate workspace without treating a historical private local path as authority, generate and verify the exact candidate manifest, then execute exactly one root PLAN-only invocation through `/opt/dashboard_RPi5/current/tools/production-release-controller.mjs` with no `--apply` or acknowledgement. Any error or ambiguity means evidence + STOP with no retry, cleanup or alternate controller path. Only a successful PLAN followed by fresh GitHub reconciliation may promote #28 to READY and allow a later separate P10 LIVE decision bound to the exact PLAN values.
...
**#191 / P10 CANDIDATE #28 — SOURCE RECONCILIATION -> MERGE GATE -> STRICT PREFLIGHT STOP:** The P9 exit reconciliation and candidate audit are complete. `ops-workflows#28` is the selected Dashboard P10 candidate for exact source `2ef105d191f7e1a97dd219745c7d6d29aabbb5ea`, but it is currently `WAITING_STRICT_READ_ONLY_PRODUCTION_PLAN_BASELINE`, not READY. This source-only reconciliation replaces the completed `#313 merge -> candidate audit` continuation with the actual current prerequisite.

The latest trusted-host preflight attempt failed closed in `READ_ONLY_PRECHECK` because the assumed local Dashboard source checkout was absent. It stopped with `PREPARATION_MUTATION_STARTED=NO`, `FETCH_PERFORMED=NO`, `PLAN_INVOCATIONS=0`, `P10_APPLY_EXECUTED=NO` and `PRODUCTION_MUTATION_STARTED=NO`. Do not retry or clean up that attempt as if authorization had been consumed, and do not treat any historical private checkout path as canonical source authority.

After this focused source reconciliation is explicitly merged and exact-main CI is freshly green, re-read `RPi5_main/main`, #191/#236, `ops-workflows#28`, Dashboard `main`/CI and the reviewed release-controller contract. Then obtain a **fresh explicit STRICT P10 PREFLIGHT owner authorization** before any protected-host/root PLAN. Under that bounded preflight, resolve or prepare one exact-source candidate workspace, run the reviewed candidate checks/manifest generation, and execute exactly one PLAN-only invocation from `/opt/dashboard_RPi5/current/tools/production-release-controller.mjs` with no `--apply` or acknowledgement. Freeze only the public-safe exact `observedCurrent`, `candidateSha256`, `targetRelease`, planned operations and apply-lock evidence. Any error or ambiguity means evidence + STOP with no retry, cleanup or alternate controller path.

Only after a successful PLAN and fresh GitHub reconciliation may #28 return to READY. The later P10 production activation remains a **separate explicit owner LIVE authorization** bound to the exact PLAN values. No P10 apply, production deploy, runner retirement or other live mutation is authorized by this source reconciliation. Phase 6 Hermes Tech work remains deferred while this #191/#236 executor lane is current.