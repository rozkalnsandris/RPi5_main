...
PREDECESSOR_SOURCE_SHA = "0659059ea712b64ad7b718db556f77c67e1d03eb"
...
UNCHANGED_ARTIFACT_KEYS = ("broker_sha256", "socket_sha256", "service_sha256")
MUTATION_BUDGET = (
    ("filesystem.weather-operator-v9-capability-module-refresh-stage", 2),
    ("filesystem.weather-operator-v9-capability-module-refresh-atomic-replace", 2),
)
...
class WeatherV9CapabilityModuleRefreshError(RuntimeError):
    pass
...
def _fail(message: str) -> None:
    raise WeatherV9CapabilityModuleRefreshError(message)
    ...
    *,
    ...
    for key in UNCHANGED_ARTIFACT_KEYS:
        if target[key] != predecessor[key]:
            _fail(f"module refresh would widen beyond module + registration: {key}")
    if target["module_sha256"] == predecessor["module_sha256"]:
        _fail("target host-capability module is not the post-#614 implementation")
    if not state_db_present:
        _fail("durable replay/state database is absent")
    if not temp_paths_absent:
        _fail("fixed module-refresh staging path already exists")
