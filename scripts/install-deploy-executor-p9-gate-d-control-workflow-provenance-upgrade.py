...
SCRIPT_RELATIVE = "scripts/install-deploy-executor-p9-gate-d-control-workflow-provenance-upgrade.py"
...
TARGET = TargetSpec(
    ...
    Path("/usr/local/lib/rozkalns-deploy-executor/deploy_executor/control_center_postcanary_adapter.py"),
...
def _preflight(expected_sha: str) -> bytes:
    ...
    if os.geteuid() != 0:
        raise UpgradeError("P9 Gate D Control workflow provenance upgrade requires root")
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Control workflow-provenance adapter. Source merge alone never authorizes --apply."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.apply:
            print(
                "P9_GATE_D_CONTROL_WORKFLOW_PROVENANCE_UPGRADE_PREFLIGHT=PASS "
                f"source_sha={args.expected_sha}"
            )
            print("P9_GATE_D_CONTROL_WORKFLOW_PROVENANCE_MUTATION=NO")
            return 0
        ...
        print(
            f"P9 Gate D Control workflow provenance upgrade refused: {exc}",
            file=sys.stderr,
        )
        return 1

    print(
        "P9_GATE_D_CONTROL_WORKFLOW_PROVENANCE_UPGRADE=PASS "
        f"source_sha={args.expected_sha}"
    )
