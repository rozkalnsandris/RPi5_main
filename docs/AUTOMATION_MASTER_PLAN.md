...
- [ ] **P10 HARDENED CONTROLLER BOOTSTRAP SOURCE / MERGE WAIT:** `ops-workflows#28` is the selected lowest-risk genuine ordinary candidate, binding `dashboard_RPi5@5f7739348f56398d0ba301c9320e1de0062838fc` to `dashboard-rpi5-production-release`. The CI-parity candidate-preparation repair is merged, and the next trusted-host preflight successfully built/staged the exact ARM64 candidate, verified its manifest and passed runtime smoke. It then classified `/opt/dashboard_RPi5/current/tools/production-release-controller.mjs` as known historical Git blob `c501bea57c0d5c35e7961ae1f1e5593a02268661` from Dashboard commit `400296591ec14c062e4c3c9fdbc95c38109ba0fd`, which predates the descriptor-safe/current-release trust boundary. The attempt stopped before `sudo`, with privileged PLAN attempts = 0, production mutation = NO and P10 APPLY = NO. That preflight authorization is consumed/non-reusable. `RPi5_main#318` merged the source-only classification that this historical blob is bootstrap-required and must never become a third ordinary PLAN allowlist identity. Current source work is the dedicated one-time bootstrap capability in PR #319: fixed exact Dashboard SHA and historical controller identity, descriptor-safe candidate consumption, full immutable release materialization, bounded lock/release/current mutation envelope, no generic shell/path/argv authority, preserved post-mutation evidence and execution disabled until a separate LIVE/root gate. #28 remains WAITING; source merge does not authorize bootstrap installation or execution. After #319 merge and exact-main CI, perform a fresh source/host reconciliation and STOP for a separate exact LIVE/root bootstrap authorization. Only after a successful bootstrap and fresh state verification may a new ordinary P10 PLAN authorization be considered.
...
### Cross-cutting Track X — owner-authorized pull deploy executor v1 — P9 EXIT GATE MET / P10 HARDENED CONTROLLER BOOTSTRAP SOURCE
...
Current supersession: canonical `RPi5_main#191` has advanced through the repaired freshness flow and proves a clean genuine P9 PASS. `RPi5_main#317` merged the P10 CI-parity preparation recipe; the next trusted-host attempt built/staged the exact Dashboard `5f7739348f56398d0ba301c9320e1de0062838fc` ARM64 candidate, verified its manifest and runtime smoke, then stopped before `sudo` because the installed current controller was historical Git blob `c501bea57c0d5c35e7961ae1f1e5593a02268661`. `RPi5_main#318` merged the fail-closed classification that this blob predates the descriptor-safe trust boundary and requires a dedicated first hardened-controller bootstrap rather than ordinary PLAN allowlisting. Therefore the current boundary is **P10 HARDENED CONTROLLER BOOTSTRAP SOURCE (#319) -> MERGE GATE -> EXACT-MAIN CI/PROVENANCE REVALIDATION -> SEPARATE LIVE/ROOT BOOTSTRAP STOP -> FRESH POST-BOOTSTRAP RECONCILIATION -> NEW ORDINARY P10 PLAN GATE**. `ops-workflows#28` remains WAITING throughout source work and must not become READY merely because the bootstrap source is merged.
...
**#191 / #236 / P10 CANDIDATE #28 — HARDENED CONTROLLER BOOTSTRAP SOURCE -> MERGE GATE -> SEPARATE LIVE/ROOT BOOTSTRAP STOP:** The P9 exit gate is met and the P10 candidate remains exact Dashboard `5f7739348f56398d0ba301c9320e1de0062838fc`. The repaired trusted-host preflight already proved candidate build/staging, manifest verification and runtime smoke, then classified the installed current controller as known historical blob `c501bea57c0d5c35e7961ae1f1e5593a02268661` and stopped before any privileged PLAN. That authorization is consumed and must not be retried or bypassed.

`RPi5_main#318` makes the bootstrap requirement canonical. Current PR #319 implements the required source-only one-time capability: it binds the exact Dashboard candidate and historical controller identity, descriptor-safely consumes candidate bytes, materializes the complete exact candidate as a root-owned immutable release, uses an exclusive fail-closed apply lock and bounded current-pointer transition, preserves post-mutation evidence, grants no generic shell/path/argv/package/service/network/credential authority, and remains execution-disabled until separately authorized. The normal production executor registry remains globally disabled and is not repurposed as the bootstrap channel.

Finish #319 only through focused source review, exact-head CI and Ready. Then STOP for explicit `MERGE RPi5_main #319`. After an owner-authorized merge, freshly require exact-main CI and source/provenance revalidation. Source merge does not authorize host installation or bootstrap execution. The next live step, if all post-merge evidence passes, is a **separate exact LIVE/root bootstrap authorization** bound to the merged source identity, exact Dashboard candidate `5f7739348f56398d0ba301c9320e1de0062838fc`, historical controller blob `c501bea57c0d5c35e7961ae1f1e5593a02268661`, exact bootstrap helper identity, exact mutation budget and fail-closed evidence policy.

Only after that bootstrap succeeds and fresh post-bootstrap verification proves the hardened current release/trust anchor may a new ordinary P10 PLAN authorization be considered. #28 remains WAITING until those gates are satisfied; no P10 APPLY, production deployment, cleanup, package installation, service/systemd mutation, credential/permission change, runner retirement or other live mutation is authorized by PR #319 or its eventual merge. Phase 6 Hermes Tech work remains deferred while this #191/#236 executor lane is current.
