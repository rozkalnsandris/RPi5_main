...
class WeatherV9CapabilityBrokerRefreshTests(unittest.TestCase):
    def test_exact_predecessor_builds_broker_only_refresh_plan(self) -> None:
        plan = valid_plan()
    ...
    def test_target_widening_beyond_broker_is_rejected(self) -> None:
        for key in ("module_sha256", "socket_sha256", "service_sha256"):
            ...
            with self.subTest(key=key), self.assertRaises(
                refresh.WeatherV9CapabilityBrokerRefreshError
            ):
                valid_plan(target_hashes=target)

    def test_unchanged_target_broker_is_rejected(self) -> None:
        with self.assertRaises(refresh.WeatherV9CapabilityBrokerRefreshError):
    ...
    def test_current_source_delta_is_broker_only_from_spent_76_predecessor(self) -> None:
    ...
    def test_contract_and_operator_preserve_fail_closed_broker_refresh_boundary(self) -> None:
        contract = json.loads(
            (ROOT / "ops/deploy/weather-operator-v9-capability-broker-refresh.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        self.assertEqual(
            contract["historical_authority"]["ops-workflows#76"],
            "SPENT_LIVE_AUTH_DO_NOT_REUSE",
        )

        operator = (ROOT / "scripts/refresh-weather-operator-v9-host-capability-broker.py").read_text(
            encoding="utf-8"
        )
