...
class WeatherV7HostCapabilityTests(unittest.TestCase):
    ...
    def test_host_capability_module_has_no_shell_or_caller_selected_execution(self) -> None:
        ...
        self.assertNotIn("[str(entrypoint)]", source)
        self.assertNotIn('"/usr/bin/git"', source)
        self.assertIn("manager-identity Git backend is not installed", source)
        self.assertIn("fixed root-owned operator replacement backend is not installed", source)
