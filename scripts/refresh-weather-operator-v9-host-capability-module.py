...
CONTRACT = ROOT / "ops/deploy/weather-operator-v9-capability-module-refresh.json"
...
MODULE_TEMP = MODULE_TARGET.with_name(".weather_operator_upgrade_v9_host_capability.py.module-refresh.tmp")
REGISTRATION_TEMP = REGISTRATION.with_name(".registration.json.module-refresh.tmp")
GIT_ENV = {"PATH": "/usr/bin:/bin", "LANG": "C", "LC_ALL": "C"}


class CapabilityModuleRefreshOperatorError(RuntimeError):
    pass


def fail(message: str) -> None:
    raise CapabilityModuleRefreshOperatorError(message)
...
def run_git(*args: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    try:
        ...
        raise CapabilityModuleRefreshOperatorError(f"git source preflight failed: {command}") from exc
...
def safe_file(path: Path, *, mode: int, max_bytes: int) -> bytes:
    try:
        ...
        raise CapabilityModuleRefreshOperatorError(f"required target is unavailable: {path.name}") from exc
    if (
...
def state_snapshot() -> tuple[int, int, int, int, int, int, int, int]:
    try:
        ...
        raise CapabilityModuleRefreshOperatorError("durable replay/state database is unavailable") from exc
    if (
...
def load_registration() -> dict[str, Any]:
    ...
    try:
        ...
        raise CapabilityModuleRefreshOperatorError("registration JSON is malformed") from exc
    try:
        return refresh.validate_registration(value)
    except refresh.WeatherV9CapabilityModuleRefreshError as exc:
        raise CapabilityModuleRefreshOperatorError(str(exc)) from exc
...
def validate_contract() -> None:
    try:
        ...
        raise CapabilityModuleRefreshOperatorError("module-refresh contract is unavailable or malformed") from exc
    expected_targets = {
        "module": str(MODULE_TARGET),
    ...
    if (
        value.get("schema") != "rozkalns.rpi5-main.weather-operator-v9-capability-module-refresh.v1"
        ...
        fail("module-refresh contract drifted")
...
def preflight_material() -> tuple[refresh.RefreshPlan, bytes, bytes, tuple[int, ...]]:
    ...
    try:
        ...
        raise CapabilityModuleRefreshOperatorError(str(exc)) from exc
    module_bytes = git_blob(sha, MODULE_SOURCE)
    registration_bytes = refresh.registration_bytes(plan.new_registration)
    return plan, module_bytes, registration_bytes, before_state


def preflight() -> dict[str, object]:
    plan, _module, _registration, _state = preflight_material()
    return {
        "schema": "rozkalns.rpi5-main.weather-operator-v9-capability-module-refresh-preflight.v1",
        "result": "PASS",
...
def write_stage(path: Path, data: bytes, mode: int) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                fail("module-refresh staging write made no progress")
            view = view[written:]
        os.fchmod(fd, mode)
        os.fchown(fd, 0, 0)
        os.fsync(fd)
...
def apply() -> dict[str, object]:
    ...
    plan, module_bytes, registration_bytes, before_state = preflight_material()
    mutation_started = False
    try:
        write_stage(MODULE_TEMP, module_bytes, 0o644)
        mutation_started = True
        write_stage(REGISTRATION_TEMP, registration_bytes, 0o600)
        os.replace(MODULE_TEMP, MODULE_TARGET)
        fsync_directory(MODULE_TARGET.parent)
        ...
        for key in refresh.ARTIFACT_KEYS:
            if observed[key] != plan.new_registration[key]:
                fail(f"post-module-refresh installed identity drifted: {key}")
        if safe_file(REGISTRATION, mode=0o600, max_bytes=64 * 1024) != registration_bytes:
            fail("post-module-refresh registration identity drifted")
        if state_snapshot() != before_state:
            fail("durable replay/state database changed during capability module refresh")
    except Exception as exc:
        raise CapabilityModuleRefreshOperatorError(
            "capability module refresh failed closed after mutation; no retry/cleanup/rollback is authorized"
        ) from exc

    return {
        "schema": "rozkalns.rpi5-main.weather-operator-v9-capability-module-refresh-receipt.v1",
        "result": "PASS",
        "source_sha": plan.source_sha,
        "predecessor_source_sha": plan.predecessor_source_sha,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Weather v9 post-#614 capability module refresh")
...
if __name__ == "__main__":
    ...
    except (CapabilityModuleRefreshOperatorError, refresh.WeatherV9CapabilityModuleRefreshError) as exc:
        print(
            json.dumps(
                {
                    "schema": "rozkalns.rpi5-main.weather-operator-v9-capability-module-refresh-receipt.v1",
