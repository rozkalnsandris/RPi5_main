...
from .weather_operator_upgrade_v9_host_capability import (
    AUTH_END,
    AUTH_START,
    AUTH_TITLE,
    WeatherV9HostCapabilityError,
    parse_authorization_issue,
)

ISSUE = 603
SOCKET_PATH = Path("/run/rozkalns-weather-operator-v9-capability/request.sock")
STATE_DIR = Path("/var/lib/rozkalns-weather-operator-v9-dispatch-caller")
ATTEMPT_LEDGER = STATE_DIR / "attempted.json"
MAX_OPEN_ISSUES = 100
MAX_REQUEST_BYTES = 4096
MAX_RESPONSE_BYTES = 16384
LEDGER_SCHEMA = "rozkalns.rpi5-main.weather-v9-dispatch-caller-attempt-ledger.v1"
RESULT_SCHEMA = "rozkalns.rpi5-main.weather-v9-dispatch-caller-result.v1"
...
class WeatherV9DispatchCallerError(RuntimeError):
    pass
...
def _fail(message: str) -> None:
    raise WeatherV9DispatchCallerError(message)
...
def _request_id_from_body(body: Any) -> str:
    ...
    try:
        ...
        raise WeatherV9DispatchCallerError("authorization payload JSON is malformed") from exc
...
def _validate_state_dir(path: Path) -> None:
    ...
    try:
        ...
        raise WeatherV9DispatchCallerError(
...
def _load_attempted(path: Path) -> dict[str, int]:
    try:
        ...
        raise WeatherV9DispatchCallerError("attempt ledger is unreadable") from exc
...
def dispatch_to_broker(
    ...
    *,
    ...
    try:
        ...
        raise WeatherV9DispatchCallerError("broker receipt is malformed") from exc
...
def source_readiness() -> Mapping[str, Any]:
    return {
        "schema": "rozkalns.rpi5-main.weather-v9-dispatch-caller.v1",
        "issue": ISSUE,
