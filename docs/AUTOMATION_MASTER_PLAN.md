...
- [ ] **P10 PREPARATION / LIVE STOP:** do not reuse #27/#8 as live execution authority. Fresh-read all READY queue candidates and choose the lowest-risk genuine ordinary deployment required by roadmap #236; before any P10 authorization prove exact source SHA/CI, target baseline, project-local deploy classification, exact adapter/helper identities, mutation count, rollback policy and exclusions. P10 remains a new explicit owner LIVE decision and is not authorized by P9 success.
...
### Cross-cutting Track X — owner-authorized pull deploy executor v1 — P9 EXIT GATE MET / P10 LIVE STOP
...
**#191 / P9 EXIT-GATE RECONCILIATION — MASTER PLAN PR -> MERGE GATE -> P10 CANDIDATE AUDIT / LIVE STOP:** This source-only change records the clean #8 `DRY_RUN_READY` exit gate and supersedes the stale `P9_EXIT_GATE=NOT_MET` continuation. Take this focused Master Plan PR through exact-head CI/review and Ready, then STOP for explicit MERGE.

After this reconciliation is merged and exact-main CI is freshly green, re-read `RPi5_main/main`, #191/#236, all open READY queue candidates, their current source/CI and target baselines, and the required project-local adapter/helper provenance. Select the **lowest-risk genuine READY ordinary deployment** available at that time; do not use a high-risk control-plane activation merely because it is convenient. The resulting P10 envelope must separately bind exact source SHA, target baseline, deploy classification, adapter/helper identities, mutation count, rollback policy and explicit exclusions.

No P10 LIVE-AUTH, mutation-capable adapter entry, production deploy, runner retirement or other live mutation is authorized by this reconciliation or by P9 success. P10 remains a separate explicit owner LIVE gate. Phase 6 Hermes Tech work remains deferred while this #191/#236 executor lane is current.