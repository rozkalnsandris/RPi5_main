# V10 encrypted backup ownership contract

## Purpose

V10 moves source ownership of the existing host-wide encrypted backup implementation from the Hermes Tech application repository to `RPi5_main`.
...
Changing any of these is outside V10 and requires a separate issue, tests, PR, migration plan, and production approval.
...
A separately approved verification must bind to the exact merged V10 commit and collect only bounded evidence:
...
Before production apply, rollback is repository-only: revert the V10 ownership commit.
