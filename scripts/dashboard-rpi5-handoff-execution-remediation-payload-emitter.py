...
EPATH='scripts/dashboard-rpi5-preverified-handoff-materializer.py'; CPATH='scripts/dashboard-rpi5-preverified-handoff-materializer-core.py'; MPATH='scripts/dashboard-rpi5-handoff-execution-bundle-materializer.py'
MFILE=REPO/MPATH
ACK='RPi5_main:EMIT-DASHBOARD-HANDOFF-EXECUTION-BYTECODE-REMEDIATION-PAYLOAD-V1'
...
def read_repo_file(path,maxn):
    f=os.open(path,os.O_RDONLY|os.O_NOFOLLOW|os.O_NONBLOCK)
    try:
        st=os.fstat(f)
        if not stat.S_ISREG(st.st_mode) or st.st_uid!=os.getuid() or st.st_size>maxn: raise Stop('repository materializer metadata mismatch')
        b=b''
        while True:
            c=os.read(f,65536)
            if not c: break
            b+=c
            if len(b)>maxn: raise Stop('repository materializer too large')
        return b
    finally: os.close(f)
def main():
    ...
    eb,cb,mb=git('rev-parse','HEAD:'+EPATH),git('rev-parse','HEAD:'+CPATH),git('rev-parse','HEAD:'+MPATH)
    materializer=read_repo_file(MFILE,1048576)
    if blob(materializer)!=mb: raise Stop('bundle materializer bytes differ from committed Git blob')
