...
### Phase 4 — Hermes Deals public-repository execution migration — CURRENT: #236 / P9 RUNTIME COMPOSITION SOURCE GATE

Phase 4 remains incomplete. The owner's later 2026-08-29 continuation correction supersedes the temporary Phase 6 priority override from #269 for current program routing: resume the cross-cutting `RPi5_main#236` pull-executor lane. Accepted post-save isolated-auth trust evidence and the merged repository-ID source binding now move the current continuation to the P9 runtime-composition source gate. This remains RPi5 executor/control-plane source work, not authorization to change Hermes Deals application behavior or any live host/runtime state.
...
- [ ] **P9 RUNTIME COMPOSITION SOURCE GATE — CURRENT:** split protocol/canary repository roles explicitly so `QUEUE_REPOSITORY=rozkalnsandris/ops-workflows` / `1328835922` remains the READY eligibility surface and `AUTHORIZATION_REPOSITORY=rozkalnsandris/deploy-authorizations` / `1350486101` becomes the LIVE-AUTH issue surface; introduce separately repository-scoped read-only queue/auth client composition and a one-shot P9 source entrypoint with fail-closed tests. This source gate must keep the installed P8 poller/runtime unchanged and keep activation, runtime binding, host wiring and production mutation disabled.
...
### Cross-cutting Track X — owner-authorized pull deploy executor v1 — P8 COMPLETE / P9 RUNTIME COMPOSITION SOURCE GATE
...
- `rozkalnsandris/deploy-authorizations` / `1350486101` is the accepted and source-bound isolated LIVE-AUTH authority surface, but host/runtime composition remains dormant;
...
- `Rozkalns Deploy Executor` App ID `4748870` remains Issues-read + Metadata-read only; accepted evidence proves it is the sole installed App on the authorization repository; future runtime must mint separately repository-scoped queue-read and authorization-read tokens rather than a generic broad token;
...
`RPi5_main#250` provides stable source repository identity, merged/reachable exact-SHA + CI proof, JIT governance freshness, genuine READY queue/source/baseline/adapter-preflight composition and final unchanged-authority verification. `RPi5_main#254` provides strict schemas/parsers for the JIT governance and sanitized Hermes baseline evidence. `RPi5_main#256` provides the fixed-path root-owned consumer provenance/placement boundary. `RPi5_main#258` provides the separately reviewed typed producer/publisher source boundary while deliberately keeping governance evidence fail-closed. `RPi5_main#260` freezes the source/tree and completeness/provenance boundary for the complete `ops-workflows` writer-surface collector and proves the remaining admin/integration inventory capability gap. `RPi5_main#263` completes the dormant isolated-surface source gate; `RPi5_main#268` supersedes the earlier connector writer assumption with owner-only writing and explicit connector exclusion; accepted #191 evidence closes the post-save trust gate; and `RPi5_main#271` binds the accepted authorization repository identity in machine source without changing P8/runtime/host/production state.

P9 live readiness now requires the remaining reviewed source composition: explicit queue-versus-LIVE-AUTH protocol roles, separately repository-scoped read-only queue/auth clients, a one-shot P9 entrypoint, exact operation-specific registry/runtime composition, a genuine READY queue and explicit owner decision. Accepted trust evidence and the repository-ID source binding are complete; they do not authorize host credential placement, P8 poller rewiring, systemd/service changes or a real P9 canary. Fresh current `RPi5_main/main`, exact-main CI and relevant cross-repository state must be resolved immediately before every consequential next step.
...
**#236 / P9 RUNTIME COMPOSITION SOURCE GATE:** implement and review the source-only repository-role/runtime composition required after accepted isolated-auth binding. The protocol and P9 canary must stop conflating the queue and LIVE-AUTH repositories: `QUEUE_REPOSITORY` remains `rozkalnsandris/ops-workflows` with stable ID `1328835922`, while `AUTHORIZATION_REPOSITORY` becomes `rozkalnsandris/deploy-authorizations` with stable ID `1350486101`. Payload `queue_repository` validation remains bound to `ops-workflows`; LIVE-AUTH issue acceptance validates the isolated authorization repository name and numeric ID.

Add separately repository-scoped read-only queue/auth client composition and a one-shot P9 source entrypoint with fail-closed synthetic tests. Do not widen GitHub App permissions or repository selection. Do not change the installed P8 poller/config/systemd/service/timer or place/rotate credentials under this source gate. Keep `activation_enabled=false`, `runtime_binding_ready=false`, `host_wiring_enabled=false`, `production_mutation_enabled=false` and `PRODUCTION_MUTATION_STARTED=false`.

The production registry remains empty/disabled until a later reviewed source gate makes the exact operation consumable without weakening P8 safety. A genuine P9 canary still requires a real READY deploy-queue item and a separate explicit owner decision; do not create dummy READY/LIVE-AUTH artifacts merely to exercise the path. Merge of any future source PR remains separately owner-authorized and does not authorize host/runtime/production mutation. Phase 6 Hermes Tech work remains deferred while this #236/P9 gate is current.