...
- [ ] Install and host-prove the RPi5_main #141 generic-maintenance Compose boundary, then recover `cvbot` through the project-owned CV deploy path and require `/health/ready`/Docker health to return healthy before installing the #140 controller path.
...
- [ ] Enable the recurring local controller only after the rollback regression gate, live-health prerequisite and one-shot `AUTO_DEPLOY_SAFE` execution canary are green.
- [ ] Disable/remove the public-repo self-hosted release runner only after the replacement execution path, rollback regression gate, live-health prerequisite and public verification are green.
...
- CV #144 installed synthetic regression evidence: `INSTALLED_PREMUTATION_ORDER=PASS`, `INSTALLED_EXPLICIT_FAILURE_PROPAGATION=PASS`, `CONDITIONAL_CONTEXT_FAILURE_PROPAGATION=PASS`, invalid-target prerequisite rejection PASS, legacy-baseline compatibility PASS, valid-target strict validation PASS, invalid-current secret rejection PASS, production unchanged at `6f13986c27d2c32c2fbcdbdbb1912bf163b8af88`, legacy helper/rule unchanged, recurring CV pull timer still `disabled/inactive`; `rozkalns-cv` issue #144 is closed Completed;
- subsequent live-health finding after the #144 regression proof: `cvbot` was `running/unhealthy` and its Docker healthcheck consistently received HTTP 404 from `/health/ready`, while local/public CV HTTP remained reachable. This operational health prerequisite is tracked separately under RPi5_main #141 and does not reopen the completed #144 rollback/pre-mutation gate;
- maintenance incident provenance from RPi5_main #123 proves the health failure was produced by the manually approved V25 catch-up: `docker compose pull --ignore-buildable` skipped the local/buildable `cvbot` image, then full-project `compose up --no-build` recreated it under newer Compose health configuration; image-tag rollback could not repair that configuration/image drift;
- generic-maintenance safety issue RPi5_main #141 owns that cross-boundary defect; source fix PR #148 merged as `f44aab15b8291dc27ea2a2b98cc52098e683bee9` with full Validate/Gitleaks/shared-policy PASS. The provenance-bound `ops/bin/rpi5-update` source remains unchanged; its existing root-owned Compose-policy helper now targets changed registry services only, excludes buildable services from recreation, uses `--no-deps`, guards changed-service config hash, and uses `--no-recreate` when no registry image changed;
...
The #144 rollback/pre-mutation prerequisite is complete and remains closed. The #140 source boundary must not be installed or exercised on the host until the RPi5_main #141 maintenance policy is installed/proven and CV is recovered through its project-owned deployment path with `cvbot` healthy. The CV pull timer remains disabled/inactive throughout those prerequisites.
...
**Phase 3 only:** install and host-prove the exact RPi5_main #141 generic-maintenance Compose-policy helper from current `main` without running the weekly updater. Keep `rozkalns-cv-pull-deploy.timer` disabled/inactive and preserve the legacy CV runner/helper. Prove the installed policy identifies the CV mixed project correctly: `cvbot` is buildable/local-only and cannot be selected for generic maintenance recreation; registry services are eligible only when their image identity changed, with `--no-deps` and a matching running-versus-desired Compose config hash; no image change must produce a `--no-recreate` path.

After that policy installation gate is green, recover `cvbot` only through the project-owned CV deployment path using fresh exact-current-main/CI and explicit manual authorization appropriate to the current deploy-impact class. Require `/health/ready` 200, Docker `healthy`, local/public CV checks PASS, production state/evidence reconciliation, and no CV pull-timer activation. Record the resulting health evidence under RPi5_main #141 and tracker #103; the completed `rozkalns-cv` #144 rollback/pre-mutation issue remains closed.

Only after #141 host proof + healthy CV recovery may Phase 3 install the reviewed #140 controller/readiness artifacts and run the separately authorized one-shot `AUTO_DEPLOY_SAFE` execution canary. Only after that one-shot execution canary is green may recurring CV pull timer activation be reviewed and separately authorized. Legacy self-hosted runner retirement remains the final Phase 3 gate and requires its own review/authorization after recurring replacement execution evidence is green.