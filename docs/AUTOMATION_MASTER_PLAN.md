...
### Phase 4 — Hermes Deals public-repository execution migration — CURRENT: #191 / P9 GENUINE READ-ONLY CANARY LIVE STOP

Phase 4 remains incomplete. Canonical current continuation is `RPi5_main#191`. Gate B source-App capability proof and Gate C least-privilege D1 credential correction are **PASS / COMPLETE**. Gate D source/host convergence and a freshness-bounded trusted baseline have also been proven through the accepted #191 evidence chain, and `ops-workflows#27` is now open **READY** as queue eligibility only. The 2026-08-31 audit checkpoint recorded merged `RPi5_main#304` and `#305`, `RPi5_main/main=366642a00fee385814f010d04964ae0a78f0430d` with exact-main Validate #731 / `33420250447`, FAST-LANE #187 / `33420250467`, and GITHUB-ONLY #175 / `33420250385` successful, `rozkalnsandris/rozkalns-control-center/main=f04601dfd47e5691c875c0935b36ff101680f4dd` with exact-head CI green, and `ops-workflows/main=db3b0ff76ee471d3b430e440a14d5cabbb1d99bc`. These identities are audit evidence only; fresh-read mutable state before any consequential continuation.

READY does not authorize execution. The baseline that supported #27 READY is limited to the fixed 300-second freshness window and is already stale for a genuine P9 run; comments or READY status do not extend it. Open `deploy-authorizations` issues #1-#4 bind the old Control source `f9b900a884bffda993197fc7fa9223c886e11a90` and are expired/non-authorizing; no LIVE-AUTH binds the current Control source. Genuine P9 has not run, no genuine-P9 StateStore lifecycle has started, and `PRODUCTION_MUTATION_STARTED=false`. A future genuine P9 therefore remains behind a new separately explicit STRICT LIVE owner envelope that must include one fresh trusted baseline, one owner-authored 600-second LIVE-AUTH bound to the then-current exact source, and exactly one P9 one-shot ending `DRY_RUN_READY`.
...
- [x] **GATE D TRUSTED BASELINE / READY ELIGIBILITY:** accepted source/host convergence, post-runtime fresh baseline PASS and #27 READY eligibility reconciliation are recorded in #191 comments `5481283344`, `5482221858`, `5482425077` and `5482473002`. This proves queue eligibility only; the baseline's fixed 300-second freshness has expired and must be freshly produced/resolved at genuine-P9 preflight.
- [ ] **P9 GENUINE READ-ONLY CANARY — CURRENT LIVE STOP:** `ops-workflows#27` is open READY eligibility for current Control source `f04601dfd47e5691c875c0935b36ff101680f4dd`, but no current-source LIVE-AUTH exists and the prior baseline is stale. After this master-plan reconciliation is merged and exact-main CI is fresh, any P9 attempt requires a new separately explicit STRICT LIVE owner envelope for one fresh baseline + one owner-authored 600-second LIVE-AUTH + exactly one P9 one-shot. During genuine P9, no owner trigger, workflow dispatch/rerun, D1 request, deploy or production mutation is allowed.
...
### Cross-cutting Track X — owner-authorized pull deploy executor v1 — P8 COMPLETE / P9 SOURCE COMPOSITION COMPLETE / READY ELIGIBILITY; GENUINE P9 LIVE STOP
...
Current supersession: canonical `RPi5_main#191` has advanced through Gate D READY eligibility. The source-App repair/diagnostic chain, final-lifecycle producer repair and executor repository-auth hardening merged through `RPi5_main#304` and `#305`; accepted public-safe #191 receipts record source-clock convergence, one-target `p9_runtime.py` convergence, a post-runtime fresh trusted baseline PASS and `ops-workflows#27` READY eligibility. At the 2026-08-31 audit checkpoint, #27 bound current Control source `f04601dfd47e5691c875c0935b36ff101680f4dd`. Its supporting baseline is already stale under the fixed 300-second freshness rule and must be newly produced/resolved for genuine P9. Historical `deploy-authorizations` #1-#4 bind old source `f9b900a884bffda993197fc7fa9223c886e11a90` and cannot authorize the current source. Genuine P9 has not run; no genuine-P9 StateStore lifecycle or production mutation has started. The current boundary is therefore **GENUINE P9 LIVE STOP after this master-plan source reconciliation**: merge remains explicit, and only after merge plus fresh exact-main CI may a new separately explicit STRICT LIVE owner envelope be considered for one fresh baseline + one owner-authored 600-second LIVE-AUTH + exactly one P9 one-shot ending `DRY_RUN_READY` with `PRODUCTION_MUTATION_STARTED=false`.
...
**#191 / P9 GENUINE READ-ONLY CANARY — SOURCE RECONCILIATION -> MERGE GATE -> LIVE STOP:** This master-plan change reconciles Phase 4 / Track X to the audited post-#304/#305 state. It is source/docs only. Take it through Draft PR, exact-head CI/review and Ready, then STOP for explicit MERGE.

After this reconciliation is merged and exact-main CI is freshly green, re-read `RPi5_main/main`, #191/#236, `ops-workflows#27`, current Control source/CI, the authorization repository and required trusted-host provenance. No existing READY comment, stale baseline or historical LIVE-AUTH is reusable. A genuine P9 requires a new separately explicit STRICT LIVE owner authorization covering exactly one fresh trusted baseline, one owner-authored 600-second LIVE-AUTH bound to the then-current exact source, and exactly one P9 one-shot.

The P9 one-shot remains read-only and fail-closed: no owner trigger, workflow dispatch/rerun, D1 request, deploy, behavior outside the reviewed P9 StateStore state machine, cleanup/rollback/alternate retry after the first authorized live operation, or production mutation is allowed. Success is only local `DRY_RUN_READY` with `PRODUCTION_MUTATION_STARTED=false`. P10 remains a separate live mutation gate. Phase 6 Hermes Tech work remains deferred while this #191/#236 P9 gate is current.