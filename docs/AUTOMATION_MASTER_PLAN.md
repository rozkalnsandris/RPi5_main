...
- [ ] **P10 PREPARATION / STRICT PREFLIGHT WAIT:** `ops-workflows#28` is the selected lowest-risk genuine ordinary candidate, binding `dashboard_RPi5@5f7739348f56398d0ba301c9320e1de0062838fc` to `dashboard-rpi5-production-release`. It remains `WAITING_STRICT_READ_ONLY_PRODUCTION_PLAN_BASELINE`, not READY. Dashboard exact-source CI is green. Two stopped preflight attempts are historical, non-reusable evidence: the first stopped before candidate preparation because an assumed local checkout was absent; the later exact-SHA attempt prepared source but used an incomplete recipe that did not explicitly rebuild `node-pty`, so staging reported `native-build-absent` and manifest generation failed before installed-controller classification or any privileged PLAN. That later receipt explicitly recorded `PLAN_VALIDATED=NO`, `PRODUCTION_MUTATION_STARTED=NO`, `P10_APPLY_EXECUTED=NO` and `RETRY_ALLOWED=NO`. The repaired source contract is `docs/OWNER_AUTHORIZED_PULL_DEPLOY_EXECUTOR_P10_PREFLIGHT.md` with machine invariants in `ops/deploy/p10-dashboard-preflight.json`: existing build prerequisites are presence-checked only; candidate preparation mirrors Dashboard CI with `npm ci --ignore-scripts`, audit, explicit source rebuild of `node-pty`, typecheck/lint/test/build, explicit native-runtime staging, manifest generate/verify and runtime smoke. After this repair is explicitly merged and resulting exact-main CI is green, obtain a fresh explicit STRICT P10 PREFLIGHT owner authorization, execute the repaired unprivileged preparation, classify the canonical installed controller by Git blob, then execute exactly one PLAN-only invocation through `/opt/dashboard_RPi5/current/tools/production-release-controller.mjs` with no `--apply` or acknowledgement. Any error or ambiguity means evidence + STOP with no retry, cleanup, package installation, controller replacement or alternate controller path. Only a successful validated PLAN followed by fresh GitHub reconciliation may promote #28 to READY and allow a later separate P10 LIVE decision bound to the exact PLAN values.
...
### Cross-cutting Track X — owner-authorized pull deploy executor v1 — P9 EXIT GATE MET / P10 STRICT PREFLIGHT WAIT
...
Current supersession: canonical `RPi5_main#191` has advanced through the repaired freshness flow and now proves a clean genuine P9 PASS. The earlier malformed/retry incident, #6 stale-baseline incident and #7 timing-expiry incident remain historical and non-reusable. `RPi5_main#308` preserved the 300-second baseline lifetime and 600-second LIVE-AUTH TTL while adding the 180-second handoff floor; `RPi5_main#312` reconciled the host convergence operator and trusted-host evidence proves repaired baseline CLI convergence. The accepted #8 sequence used a fresh baseline with 299 seconds remaining, an owner-authored/app-null LIVE-AUTH three seconds later, and exactly one P9 one-shot ending `DRY_RUN_READY` with mutation dispatch, result writer and production mutation all false. Therefore `P9_EXIT_GATE=MET` and P9 no longer blocks P10. `ops-workflows#28` is now bound to exact Dashboard source `5f7739348f56398d0ba301c9320e1de0062838fc`. The latest P10 preflight stopped before `sudo` because the candidate preparation omitted the explicit CI-required `node-pty` source rebuild; the repaired source contract is being reviewed in `RPi5_main#317`. The current boundary is **P10 RECIPE REPAIR/MERGE -> #28 WAITING -> STRICT READ-ONLY PRODUCTION PLAN PREFLIGHT -> FRESH GITHUB RECONCILIATION -> SEPARATE P10 LIVE STOP**.
...
**#191 / #236 / P10 CANDIDATE #28 — P10 PREFLIGHT RECIPE REPAIR -> MERGE GATE -> STRICT PREFLIGHT STOP:** The P9 exit gate is met. `ops-workflows#28` is the selected Dashboard P10 candidate for exact source `5f7739348f56398d0ba301c9320e1de0062838fc` and remains `WAITING_STRICT_READ_ONLY_PRODUCTION_PLAN_BASELINE`, not READY.

Two prior preflight attempts remain historical and non-reusable. The first stopped before candidate preparation because an assumed local checkout was absent. The later exact-SHA attempt reached candidate build but used the incomplete `npm run check` preparation shape; `package-terminal-native-runtime.mjs --if-built` reported `native-build-absent`, then production candidate manifest generation failed on missing `apps/terminal-agent/dist/native/node-pty`. It stopped before installed-controller classification and before any `sudo` PLAN with `PLAN_VALIDATED=NO`, `PRODUCTION_MUTATION_STARTED=NO`, `P10_APPLY_EXECUTED=NO` and `RETRY_ALLOWED=NO`.

`RPi5_main#317` freezes the repair in `docs/OWNER_AUTHORIZED_PULL_DEPLOY_EXECUTOR_P10_PREFLIGHT.md` and `ops/deploy/p10-dashboard-preflight.json`, with regression coverage in `tests/test-deploy-executor-p10-preflight-recipe.py`. The normative unprivileged candidate sequence mirrors Dashboard CI: `npm ci --ignore-scripts` -> dependency audit -> explicit `npm_config_build_from_source=true npm rebuild node-pty --dangerously-allow-all-scripts --foreground-scripts` -> typecheck/lint/test/build -> explicit native-runtime staging -> candidate manifest generate/verify -> production runtime smoke. Existing Node 24/native build prerequisites are presence-checked only; package installation/upgrade is outside this preflight.

After this focused source repair is explicitly merged and resulting exact-main CI is freshly green, re-read `RPi5_main/main`, #191/#236, `ops-workflows#28`, Dashboard exact `5f7739348f56398d0ba301c9320e1de0062838fc`/CI and the reviewed release-controller contract. Then obtain a **fresh explicit STRICT P10 PREFLIGHT owner authorization**. Under that bounded authorization, prepare one fresh exact-SHA candidate using the frozen CI-parity recipe, classify `/opt/dashboard_RPi5/current/tools/production-release-controller.mjs` by Git blob against the two reviewed controller identities, select exactly one invocation form before `sudo`, and execute at most one PLAN-only call with no `--apply` or acknowledgement. Require parsed/validated PLAN JSON; exit zero alone is insufficient. Any error or ambiguity means evidence + STOP with no retry, cleanup, package installation, controller replacement or alternate controller path.

Only after a successful PLAN and fresh GitHub reconciliation may #28 return to READY. The later P10 production activation remains a **separate explicit owner LIVE authorization** bound to the exact PLAN values. No P10 apply, production deploy, runner retirement or other live mutation is authorized by this source repair. Phase 6 Hermes Tech work remains deferred while this #191/#236 executor lane is current.
