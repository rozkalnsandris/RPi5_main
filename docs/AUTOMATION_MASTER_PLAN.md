...
Current supersession after PR #371 (2026-09-05): canonical `RPi5_main#191` and merged PR #371 now bind the Hermes Deals origin broker first-install source at current reviewed checkpoint `32655619fca01105f872a4a2c44c41edc373b4dd`. Exact-main Validate `33959346010`, FAST-LANE `33959346085` and GITHUB-ONLY `33959346006` are green, and fresh `hermes-deals/main=41bfdd21ce93c478bd294c82eb8cd67405c82046` still carries the reviewed origin helper blob `51bb23cc6c2083ab7c8b4e81ba82dd880e46d673`. One bounded default-mode installer preflight on an exact clean trusted-RPi5 checkout failed closed before mutation because the fixed source GitHub App credential target is absent; public-safe receipt is #191 comment `5551180411`. The preflight did not read credential content, mutate credentials, run the helper, perform systemd work or invoke installer `--apply`.

Issue #372 is therefore the current **SOURCE-ONLY** prerequisite lane. It adds a capability-specific first-install credential provisioner with hidden multiline TTY input, exact-source revalidation, fixed `/etc/rozkalns-hermes-deals-origin-broker/source-github-app.pem` target, `root:root 0600`, optional fixed `root:root 0700` parent creation, `O_CREAT|O_EXCL|O_NOFOLLOW|O_CLOEXEC`, and no overwrite/rotation/retry/rollback/cleanup. The provisioner performs no GitHub API/token mint, App permission/repository-selection change, broker install, helper/audit execution or systemd action. Source readiness does not authorize credential placement. The current sequence is **#372 SOURCE -> explicit MERGE gate -> fresh exact-main CI/provenance -> separate one-shot STRICT credential-placement LIVE gate (max one parent-dir create-if-absent + one credential-file create) -> fresh bounded read-only broker installer preflight -> later separate broker `--apply` LIVE gate**. Genuine audit dispatch, P10 production execution and runner retirement remain later separately gated operations. No historical credential or installer authorization may be reused.
