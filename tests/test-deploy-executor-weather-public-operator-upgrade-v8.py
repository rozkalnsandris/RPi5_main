...
MODULE = ROOT / "ops/lib/deploy_executor/weather_public_runtime_operator_upgrade_v8.py"
...
class WeatherOperatorEntrypointUpgradeV8Tests(unittest.TestCase):
    def test_source_contract_is_fixed_zero_input_and_inactive(self) -> None:
        ready = upgrade.source_readiness()
        self.assertEqual(ready["issue"], 597)
        self.assertEqual(ready["caller_authority"], ())
        self.assertEqual(
            ready["trusted_upgrade_checkout"],
            "RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v8-trusted",
        )
        ...
        self.assertNotIn("upgrade v7", source)
        for forbidden in (
            "argparse",
            "--path",
            "--command",
    ...
    def test_upgrade_contract_pins_exact_transition(self) -> None:
        ...
        self.assertEqual(contract["new_sha256"], "c9cddce6890039e50a80b16ce3cafed67a0273fea3e9dbfff29efd00deccb8a9")
    ...
    def test_checkout_contract_preserves_v7_and_uses_new_identity(self) -> None:
        contract = json.loads(CHECKOUT_CONTRACT.read_text(encoding="utf-8"))
        target = "RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v8-trusted"
        auth_sha = "EXPLICIT_WEATHER_OPERATOR_UPGRADE_V8_LIVE_EXACT_RPI5_MAIN_SHA"
        ...
        v7 = historical["RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v7-trusted"]
        self.assertFalse(v7["mutation_allowed"])
        self.assertFalse(v7["cleanup_allowed"])
        self.assertFalse(v7["authority_source"])
