...
SCRIPT_RELATIVE = "scripts/install-deploy-executor-p9-gate-d-api-2026-compat-upgrade.py"
...
def _preflight(expected_sha: str) -> dict[TargetSpec, bytes]:
    ...
    if os.geteuid() != 0:
        raise UpgradeError("P9 Gate D API 2026 compatibility upgrade requires root")
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Preflight, and only with --apply replace, the one reviewed P9 Gate D "
            "GitHub REST API 2026 compatibility target. Source merge alone never "
            "authorizes --apply."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.apply:
            print(
                "P9_GATE_D_API_2026_COMPAT_UPGRADE_PREFLIGHT=PASS "
                f"source_sha={args.expected_sha}"
            )
            print("P9_GATE_D_API_2026_COMPAT_MUTATION=NO")
            return 0
        ...
        print(f"P9 Gate D API 2026 compatibility upgrade refused: {exc}", file=sys.stderr)
        return 1

    print(
        "P9_GATE_D_API_2026_COMPAT_UPGRADE=PASS "
        f"source_sha={args.expected_sha}"
    )
