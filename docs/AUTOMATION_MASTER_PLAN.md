...
- [ ] Fix and regression-prove the cross-repository #140 evidence-directory contract before installing or executing the reviewed controller path. Current RPi5 controller source creates `rozkalns-cv-auto-deploy-*`, while the current CV root pull wrapper accepts only `rozkalns-cv-main-deploy-*`; preserve the wrapper's path-bounding security restriction, make the controller producer use the accepted contract, and add an end-to-end/cross-repository regression that fails if these names drift again.
- [ ] Only after that contract gate is merged/current, install and host-prove the reviewed #140 controller/readiness artifacts with the recurring timer still disabled/inactive; use the merged `ops/bin/rozkalns-cv-controller-activate` host operator only after exact-source/CI/artifact revalidation and do not run the unrelated V26 maintenance updater or weekly maintenance in this Phase 3 step.
...
The #144 rollback/pre-mutation and health-recovery gate is complete. CV production is reconciled to exact current `f5431265232f356fa27f6204f0cba56e1e730928`, transactional public checks passed, readiness is `CURRENT`, and the recurring CV pull timer remains disabled/inactive. The first incomplete Phase 3 prerequisite is now the cross-repository controller/wrapper evidence-directory contract fix and regression proof; #140 host activation is blocked until that gate is green.
...
**Phase 3 only:** do not run the merged `ops/bin/rozkalns-cv-controller-activate` host operator and do not install/exercise the #140 controller path yet. First fix and regression-prove the cross-repository evidence-directory contract: the RPi5 controller must create an evidence directory that the current CV pull wrapper accepts, while preserving the wrapper's existing path-bounding security restriction. Prefer changing the producer/controller prefix to an accepted `rozkalns-cv-main-deploy-*` form rather than broadening the root wrapper allow-pattern. Add a focused regression that verifies the exact controller-produced path contract against the current pull wrapper and fails if either repo changes incompatibly; keep all non-READY states non-mutating.

After that source fix is merged/current and both repositories' relevant CI/contracts are green, re-read this plan and then install/host-prove the exact reviewed #140 controller/readiness artifacts from current `RPi5_main/main`. Keep `rozkalns-cv-pull-deploy.timer` disabled/inactive, preserve the legacy self-hosted release path, revalidate exact RPi5_main source/CI plus installed CV preflight/classifier/pull-library/pull-wrapper identities and #141 maintenance policy, and do not activate the unrelated V26 maintenance updater or execute weekly maintenance as part of this Phase 3 host step.
