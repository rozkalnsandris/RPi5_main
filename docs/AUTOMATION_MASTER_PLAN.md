...
- [ ] Fix and regression-prove CV pull rollback/pre-mutation prerequisite handling from `rozkalns-cv` issue #144 before installing or executing the #140 controller path.
...
- rollback blocker #144: target-only `CLIENT_KEY_SECRET` validation currently occurs after `MUTATION_STARTED=true`, and rollback reuses newer target helper semantics against the restored older baseline. Recurring/local automatic execution remains unauthorized until target prerequisites are pre-mutation and rollback behavior is baseline-aware and regression-proven;
...
The #140 source boundary must not be installed or exercised on the host until `rozkalns-cv` issue #144 is fixed and the exact installed pull transport passes a non-destructive rollback/pre-mutation regression gate with the recurring timer still disabled/inactive.
...
**Phase 3 only:** fix `rozkalns-cv` issue #144 before installing or exercising the #140 controller source on the host. The pull path must validate target-only runtime prerequisites before `MUTATION_STARTED=true` and rollback must evaluate the restored baseline's requirements rather than blindly reusing newer target-only prerequisite semantics. Add focused regression tests, run full CV CI, merge only from current `main`, then install the exact pull-library/wrapper artifacts and run a non-destructive rollback/pre-mutation regression proof with `rozkalns-cv-pull-deploy.timer` still disabled/inactive.

Only after #144 is merged and that host regression gate is green may Phase 3 install the reviewed #140 controller/readiness artifacts and run the separately authorized one-shot `AUTO_DEPLOY_SAFE` execution canary. Require fresh exact-current-main/CI/pull-artifact revalidation, transactional rollback-capable MIME/cache/nosniff/CSP evidence, final production at the exact target, post-deploy readiness `CURRENT`, and the legacy self-hosted release path unchanged.

Only after that one-shot execution canary is green may recurring timer activation be reviewed and separately authorized. Legacy self-hosted runner retirement remains the final Phase 3 gate and requires its own review/authorization after recurring replacement execution evidence is green.
