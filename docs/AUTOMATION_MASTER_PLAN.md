...
- [x] Fix and regression-prove the cross-repository #140 evidence-directory contract before installing or executing the reviewed controller path. RPi5_main PR #157 merged as `d1437461126e4118f20cd503135aa9b190bb4b09`; the fixed controller producer now uses `rozkalns-cv-main-deploy-auto-*`, the CV root wrapper keeps its existing bounded `rozkalns-cv-main-deploy-*` restriction unchanged, and fixed PR head `d1a4628a31adf08548673ba9919faa835c9a0840` passed Validate run #398 (`31434508541`).
...
- [ ] Enable the recurring local controller only after the rollback regression gate, healthy recovery prerequisite, cross-repository evidence contract gate and one-shot `AUTO_DEPLOY_SAFE` execution canary are green.
...
- #140 host-activation operator source merged on RPi5_main as `321ff3bc593d1d2dc074c59a4f76340bb807811a`, adding `ops/bin/rozkalns-cv-controller-activate` plus focused tests. This source merge does not authorize or prove host activation;
- 2026-08-10 end-to-end audit found a P1 cross-repository contract mismatch not covered by the repository-local green CI suites: RPi5 controller blob `b3599d308f10f432af09faaf7a5af28c07a2cdd5` created `$EVIDENCE_ROOT/rozkalns-cv-auto-deploy-${TARGET_SHA:0:12}.XXXXXXXX`, while CV pull-wrapper blob `ddaa8c7f8c0776e77be18b2cd5ea8a9489900e70` accepted evidence only under `$PULL_EVIDENCE_ROOT/rozkalns-cv-main-deploy-*`; this was fail-closed before production mutation;
- cross-repository evidence contract fix: RPi5_main PR #157 merged as `d1437461126e4118f20cd503135aa9b190bb4b09`; fixed controller producer basename `rozkalns-cv-main-deploy-auto-${TARGET_SHA:0:12}.XXXXXXXX`, CV wrapper bounded allow-pattern unchanged, cross-repository activation proof added, fixed PR head `d1a4628a31adf08548673ba9919faa835c9a0840`, Validate run #398 (`31434508541`) PASS;
...
The #144 rollback/pre-mutation and health-recovery gate is complete. CV production is reconciled to exact current `f5431265232f356fa27f6204f0cba56e1e730928`, transactional public checks passed, readiness is `CURRENT`, and the recurring CV pull timer remains disabled/inactive. The cross-repository controller/wrapper evidence-directory contract gate is now complete through PR #157; the first incomplete Phase 3 prerequisite is the reviewed #140 controller/readiness host-install proof with the recurring timer still disabled/inactive.
...
**Phase 3 only:** the cross-repository #140 evidence-directory contract gate is merged/current through RPi5_main PR #157 (`d1437461126e4118f20cd503135aa9b190bb4b09`) with fixed-head Validate #398 PASS. The first incomplete gate is now the reviewed #140 controller/readiness **host-install proof**. Before any host mutation, re-read current `RPi5_main/main` and require a successful exact-main push CI run, then revalidate exact installed CV preflight/classifier/pull-library/pull-wrapper identities, the controller↔wrapper evidence contract, CV main/production reconciliation at `f5431265232f356fa27f6204f0cba56e1e730928`, #141 maintenance-policy identity, timer/service state, and local/public health.

After those preconditions are green and only under separate explicit host-apply authorization, run the merged `ops/bin/rozkalns-cv-controller-activate` operator. The host step remains non-deploying: install/prove only the reviewed controller/readiness/service/timer artifacts, keep `rozkalns-cv-pull-deploy.timer` disabled/inactive, preserve production and the legacy self-hosted release path, and do not activate the unrelated V26 maintenance updater or execute weekly maintenance.
...
Only after that one-shot `AUTO_DEPLOY_SAFE` execution canary is green may recurring CV pull timer activation be reviewed and separately authorized. Legacy self-hosted runner retirement remains the final Phase 3 gate and requires its own review/authorization after recurring replacement execution evidence is green.
