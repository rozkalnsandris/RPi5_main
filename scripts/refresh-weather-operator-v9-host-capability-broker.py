...
from deploy_executor import weather_operator_v9_capability_broker_refresh as refresh

ORIGIN = "https://github.com/rozkalnsandris/RPi5_main.git"
CONTRACT = ROOT / "ops/deploy/weather-operator-v9-capability-broker-refresh.json"
...
class CapabilityBrokerRefreshOperatorError(RuntimeError):
    pass


def fail(message: str) -> None:
    raise CapabilityBrokerRefreshOperatorError(message)
...
def run_git(*args: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    try:
        ...
        raise CapabilityBrokerRefreshOperatorError(f"git source preflight failed: {command}") from exc
...
def safe_file(path: Path, *, mode: int, max_bytes: int) -> bytes:
    try:
        ...
        raise CapabilityBrokerRefreshOperatorError(f"required target is unavailable: {path.name}") from exc
    if (
...
def state_snapshot() -> tuple[int, int, int, int, int, int, int, int]:
    try:
        ...
        raise CapabilityBrokerRefreshOperatorError("durable replay/state database is unavailable") from exc
    if (
...
def load_registration() -> dict[str, Any]:
    ...
    try:
        ...
        raise CapabilityBrokerRefreshOperatorError("registration JSON is malformed") from exc
    try:
        return refresh.validate_registration(value)
    except refresh.WeatherV9CapabilityBrokerRefreshError as exc:
        raise CapabilityBrokerRefreshOperatorError(str(exc)) from exc
...
def validate_contract() -> None:
    try:
        ...
        raise CapabilityBrokerRefreshOperatorError("broker-refresh contract is unavailable or malformed") from exc
    expected_targets = {
    ...
    if (
        value.get("schema") != "rozkalns.rpi5-main.weather-operator-v9-capability-broker-refresh.v1"
        ...
        fail("broker-refresh contract drifted")
...
def preflight() -> dict[str, object]:
    ...
    return {
        "schema": "rozkalns.rpi5-main.weather-operator-v9-capability-broker-refresh-preflight.v1",
        "result": "PASS",
...
def write_stage(path: Path, data: bytes, mode: int) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                fail("broker-refresh staging write made no progress")
            view = view[written:]
        os.fchmod(fd, mode)
        os.fchown(fd, 0, 0)
        os.fsync(fd)
    ...
    try:
        ...
        for key in refresh.ARTIFACT_KEYS:
            if observed[key] != plan.new_registration[key]:
                fail(f"post-broker-refresh installed identity drifted: {key}")
        if safe_file(REGISTRATION, mode=0o600, max_bytes=64 * 1024) != registration_bytes:
            fail("post-broker-refresh registration identity drifted")
        if state_snapshot() != before_state:
            fail("durable replay/state database changed during capability broker refresh")
    except Exception as exc:
        raise CapabilityBrokerRefreshOperatorError(
            "capability broker refresh failed closed after mutation; no retry/cleanup/rollback is authorized"
        ) from exc

    return {
        "schema": "rozkalns.rpi5-main.weather-operator-v9-capability-broker-refresh-receipt.v1",
        "result": "PASS",
        "source_sha": plan.source_sha,
        "predecessor_source_sha": plan.predecessor_source_sha,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Weather v9 post-#76 capability broker refresh")
...
if __name__ == "__main__":
    ...
    except (CapabilityBrokerRefreshOperatorError, refresh.WeatherV9CapabilityBrokerRefreshError) as exc:
        print(
            json.dumps(
                {
                    "schema": "rozkalns.rpi5-main.weather-operator-v9-capability-broker-refresh-receipt.v1",
