...
SCRIPT_RELATIVE = "scripts/install-deploy-executor-p9-source-repair-upgrade.py"
...
def _preflight(expected_sha: str) -> dict[TargetSpec, bytes]:
    ...
    if os.geteuid() != 0:
        raise UpgradeError("P9 source repair upgrade requires root")
...
def _write_fd_all(fd: int, data: bytes) -> None:
    ...
    while written < len(view):
        ...
        if count <= 0:
            raise UpgradeError("short write while replacing reviewed P9 source")
        written += count
...
def _replace_exact_target(spec: TargetSpec, reviewed_bytes: bytes) -> None:
    ...
    try:
        ...
        # Authorized two-target P9 source repair mutation begins at the first ftruncate.
        os.lseek(fd, 0, os.SEEK_SET)
        os.ftruncate(fd, 0)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Preflight, and only with --apply replace, the two reviewed P9 source-repair "
            "targets. Source merge alone never authorizes --apply."
        )
    )
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="perform the separately owner-authorized two-target live mutation",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.apply:
            print(f"P9_SOURCE_REPAIR_UPGRADE_PREFLIGHT=PASS source_sha={args.expected_sha}")
            print("P9_SOURCE_REPAIR_MUTATION=NO")
            return 0
        ...
        print(f"P9 source repair upgrade refused: {exc}", file=sys.stderr)
        return 1

    print(f"P9_SOURCE_REPAIR_UPGRADE=PASS source_sha={args.expected_sha}")
    print("TARGETS_REPLACED=2")
