...
CONTRACT = ROOT / "ops/deploy/weather-public-runtime-operator-upgrade-v9-host-capability-installer.json"
TARGET_ROOT = Path("/usr/local/libexec/rozkalns-weather-operator-v9-capability")
...
ARTIFACTS = (
    ...
    ("ops/systemd/rozkalns-weather-operator-v9-privileged-broker.socket",
     SYSTEMD_ROOT / SOCKET_NAME, 0o644),
...
def manager_identity(manager: Path) -> tuple[int, int]:
    try:
        info = manager.lstat()
        parent_info = manager.parent.lstat()
    except OSError as exc:
    ...
    if not (0 < uid <= MAX_UID) or not (0 < gid <= MAX_UID):
        fail("canonical manager checkout owner identity is invalid")
    if not stat.S_ISDIR(parent_info.st_mode) or manager.parent.is_symlink() or parent_info.st_uid != uid:
...
def render_service_unit(data: bytes, manager: Path) -> bytes:
    if data.count(SERVICE_MANAGER_PARENT_TOKEN) != 1:
        fail("Weather-v9 broker service manager-parent placeholder drifted")
    ...
    if SERVICE_MANAGER_PARENT_TOKEN in rendered:
        fail("Weather-v9 broker service manager-parent placeholder was not fully rendered")
    return rendered
...
def installed_bytes(source: str, manager: Path) -> bytes:
    ...
    return render_service_unit(data, manager) if source == SERVICE_SOURCE else data
...
def preflight() -> dict[str, object]:
    ...
    if contract.get("schema") != "rozkalns.rpi5-main.weather-operator-upgrade-v9-host-capability-installer.v1":
    ...
    if type(fixed_targets) is not dict:
    ...
    for target in (
        TARGET_ROOT, BROKER_TARGET, CONFIG_ROOT, REGISTRATION,
        SYSTEMD_ROOT / SOCKET_NAME, SYSTEMD_ROOT / SERVICE_NAME, STATE_ROOT, STATE_DB,
    ):
        if target.exists() or target.is_symlink():
            fail(f"first-install target already exists: {target}")
    return {
        "schema": "rozkalns.rpi5-main.weather-operator-upgrade-v9-host-capability-installer-preflight.v1",
        "result": "PASS",
        "source_sha": sha,
...
def apply() -> dict[str, object]:
    ...
    try:
        ...
        registration = {
            ...
            "manager_gid": manager_gid,
            "artifact_count": len(ARTIFACTS),
            "module_sha256": hashes[str(PACKAGE_ROOT / "weather_operator_upgrade_v9_host_capability.py")],
        ...
        raise InstallError(
            "host-capability installation failed closed after mutation; "
            "no retry/cleanup/rollback is authorized"
        ) from exc

    return {
        "schema": "rozkalns.rpi5-main.weather-operator-upgrade-v9-host-capability-install-receipt.v1",
        "result": "PASS",
        "source_sha": sha,
        "manager_checkout": str(manager),
        "artifact_count": len(ARTIFACTS),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Install the fixed Weather v9 privileged host capability")
