...
PREDECESSOR_SOURCE_SHA = "68ef2b73dc8600dd63cf2a3b6920814e27313600"
...
class WeatherV9CapabilityRefreshError(RuntimeError):
    pass
...
def _fail(message: str) -> None:
    raise WeatherV9CapabilityRefreshError(message)
    ...
    *,
    ...
    for key in UNCHANGED_ARTIFACT_KEYS:
        if target[key] != predecessor[key]:
            _fail(f"refresh would widen beyond broker + registration: {key}")
    if target["broker_sha256"] == predecessor["broker_sha256"]:
        _fail("target broker is not the post-recovery implementation")
    if not state_db_present:
        _fail("durable replay/state database is absent")
    if not temp_paths_absent:
        _fail("fixed refresh staging path already exists")
