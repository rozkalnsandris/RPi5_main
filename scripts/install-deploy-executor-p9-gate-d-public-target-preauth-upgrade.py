...
SCRIPT_RELATIVE = "scripts/install-deploy-executor-p9-gate-d-public-target-preauth-upgrade.py"
...
def _preflight(expected_sha: str) -> dict[TargetSpec, bytes]:
    ...
    if os.geteuid() != 0:
        raise UpgradeError("P9 Gate D public-target pre-auth upgrade requires root")
...
def _replace_exact_target(spec: TargetSpec, reviewed_bytes: bytes) -> None:
    ...
    try:
        ...
        # Authorized two-target Gate D public-target pre-auth mutation begins here.
        os.lseek(fd, 0, os.SEEK_SET)
        os.ftruncate(fd, 0)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Preflight, and only with --apply replace, the two reviewed P9 Gate D "
            "public-target pre-auth targets. Source merge alone never authorizes --apply."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.apply:
            print(
                "P9_GATE_D_PUBLIC_TARGET_PREAUTH_UPGRADE_PREFLIGHT=PASS "
                f"source_sha={args.expected_sha}"
            )
            print("P9_GATE_D_PUBLIC_TARGET_PREAUTH_MUTATION=NO")
            return 0
        ...
        print(f"P9 Gate D public-target pre-auth upgrade refused: {exc}", file=sys.stderr)
        return 1

    print(
        "P9_GATE_D_PUBLIC_TARGET_PREAUTH_UPGRADE=PASS "
        f"source_sha={args.expected_sha}"
    )
