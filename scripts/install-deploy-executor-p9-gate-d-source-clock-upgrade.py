...
SCRIPT_RELATIVE = "scripts/install-deploy-executor-p9-gate-d-source-clock-upgrade.py"
...
def _preflight(expected_sha: str) -> bytes:
    ...
    if os.geteuid() != 0:
        raise UpgradeError("P9 Gate D source-clock upgrade requires root")
...
def _write_fd_all(fd: int, data: bytes) -> None:
    ...
    while written < len(view):
        ...
        if count <= 0:
            raise UpgradeError("short write while replacing reviewed P9 source auth")
        written += count
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "P9 source-App auth module. Source merge alone never authorizes --apply."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.apply:
            print(
                "P9_GATE_D_SOURCE_CLOCK_UPGRADE_PREFLIGHT=PASS "
                f"source_sha={args.expected_sha}"
            )
            print("P9_GATE_D_SOURCE_CLOCK_MUTATION=NO")
            return 0
        ...
        print(f"P9 Gate D source-clock upgrade refused: {exc}", file=sys.stderr)
        return 1

    print(
        "P9_GATE_D_SOURCE_CLOCK_UPGRADE=PASS "
        f"source_sha={args.expected_sha}"
    )
