...
def _load_receipt(path: Path, *, expected_source_sha: str) -> RuntimeArtifactReceipt:
    st = _safe_regular(path, max_bytes=64 * 1024)
    try:
        raw = path.read_bytes()
        if len(raw) != st.st_size:
            _fail("runtime receipt size changed during read")
        value = json.loads(raw.decode("utf-8", "strict"))
    except (OSError, UnicodeError, json.JSONDecodeError):
        _fail("runtime receipt is unreadable")
    if not isinstance(value, dict):
        _fail("runtime receipt must be a JSON object")
    try:
...
def _incoming_receipt(expected_source_sha: str) -> RuntimeArtifactReceipt:
    try:
    ...
    if entries != {INCOMING_RECEIPT.name, INCOMING_ARTIFACT.name}:
...
def _cache_state(receipt: RuntimeArtifactReceipt) -> str:
...
def _runtime_state(receipt: RuntimeArtifactReceipt) -> str:
    ...
    return "EXACT" if classify_runtime_identity(observed, receipt=receipt) == "runtime_materialization_source_ready" else "CONFLICT"
...
def build_transport_plan(expected_source_sha: str) -> RuntimeTransportPlan:
    receipt = _incoming_receipt(expected_source_sha)
    cache_state = _cache_state(receipt)
...
def _publish_cache(receipt: RuntimeArtifactReceipt) -> None:
    if _cache_state(receipt) != "ABSENT":
...
def apply_transport_plan(plan: RuntimeTransportPlan) -> Mapping[str, Any]:
    ...
    if public_plan(build_transport_plan(plan.source_sha)) != public_plan(plan):
        _fail("private runtime transport plan drifted before mutation")
    executed: list[str] = []
    if plan.cache_state == "ABSENT":
        _publish_cache(plan.receipt)
...
def _fixed_registry() -> OperationRegistry:
    operation = OperationSpec(
        ...
        preflight=(
            ...
            "fixed incoming runtime artifact handoff is root-controlled and byte-exact",
...
class ConcreteRuntimeRevalidator:
    ...
    def revalidate(self, issue_number: int) -> CanonicalRuntimeEvidence:
        ...
        build_transport_plan(rpi.source_sha)
...
def run_privileged_runtime_materialization(authorization_issue_number: int) -> Mapping[str, object]:
    ...
    try:
        ...
        _stable(first, final)
        plan = build_transport_plan(final.rpi5_main_sha)
        ...
        preconsume_plan = build_transport_plan(preconsume.rpi5_main_sha)
