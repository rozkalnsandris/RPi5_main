...
MODULE = ROOT / "ops/lib/deploy_executor/weather_public_runtime_operator_upgrade_v6.py"
ENTRYPOINT_SOURCE = ROOT / "ops/bin/rozkalns-weather-public-runtime-operator-upgrade-v6"
UPGRADE_CONTRACT = ROOT / "ops/deploy/weather-public-runtime-operator-upgrade-v6.json"
CHECKOUT_CONTRACT = ROOT / "ops/deploy/rpi5-main-weather-public-runtime-operator-upgrade-v6-trusted-checkout-bootstrap.json"
sys.path.insert(0, str(ROOT / "ops/lib"))

from deploy_executor import weather_public_runtime_operator_upgrade_v6 as upgrade
...
class WeatherOperatorEntrypointUpgradeV6Tests(unittest.TestCase):
    def test_source_contract_is_fixed_zero_input_and_inactive(self) -> None:
        ready = upgrade.source_readiness()
        self.assertEqual(ready["issue"], 525)
        self.assertEqual(ready["caller_authority"], ())
        self.assertEqual(
            ready["trusted_upgrade_checkout"],
            "RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v6-trusted",
        )
    ...
    def test_checkout_contract_preserves_v5_and_uses_new_identity(self) -> None:
        contract = json.loads(CHECKOUT_CONTRACT.read_text(encoding="utf-8"))
        target = "RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v6-trusted"
        auth_sha = "EXPLICIT_WEATHER_OPERATOR_UPGRADE_V6_LIVE_EXACT_RPI5_MAIN_SHA"
        ...
        v5 = historical["RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v5-trusted"]
        self.assertFalse(v5["mutation_allowed"])
        self.assertFalse(v5["cleanup_allowed"])
        self.assertFalse(v5["authority_source"])
