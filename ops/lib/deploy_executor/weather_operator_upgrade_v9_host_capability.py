...
OPERATION_ID = "rpi5-main.weather-operator-upgrade-v9.v1"
ADAPTER_ID = OPERATION_ID
TARGET_ALIAS = "rpi5-main-weather-operator-upgrade-v9"
AUTHORIZATION_CLASS = "STRICT"
ROLLBACK_POLICY = "NONE"
BASELINE_RESOLVER_ID = "rpi5-main.weather-operator-upgrade-v9-baseline.v1"
ENTRYPOINT = "ops/bin/rozkalns-weather-public-runtime-operator-upgrade-v9"
UPGRADE_MODULE = "ops/lib/deploy_executor/weather_public_runtime_operator_upgrade_v9.py"
TRUSTED_CHECKOUT_NAME = "RPi5_main-weather-public-runtime-operator-upgrade-v9-trusted"
PRESERVED_V8_CHECKOUT_NAME = "RPi5_main-weather-public-runtime-operator-upgrade-v8-trusted"
PRESERVED_V8_SHA = "77482a16fb77338d87b92a39ff8c63a5b6ce142d"
REVIEWED_ORIGIN = "https://github.com/rozkalnsandris/RPi5_main.git"
MINIMUM_REVIEWED_ANCESTOR = PRESERVED_V8_SHA
TARGET_PATH = Path(
    "/usr/local/libexec/rozkalns-weather-public-runtime-operator/"
    "deploy_executor/weather_public_runtime_operator.py"
)
...
AUTH_START = "<!-- rozkalns-weather-v9-live-auth:v1 -->"
AUTH_END = "<!-- /rozkalns-weather-v9-live-auth:v1 -->"
AUTH_SCHEMA = "rozkalns.rpi5-main.weather-operator-upgrade-v9-live-auth.v1"
AUTH_TITLE = "[LIVE-AUTH][PENDING] rpi5-main-weather-operator-upgrade-v9"
...
SOCKET_UNIT = "rozkalns-weather-operator-v9-privileged-broker.socket"
SERVICE_UNIT = "rozkalns-weather-operator-v9-privileged-broker@.service"
...
REQUIRED_EXCLUSIONS = (
    ...
    "no v7 authorization or checkout authority reuse",
)


class WeatherV9HostCapabilityError(RuntimeError):
    pass


@dataclass(frozen=True)
class WeatherV9Authorization:
    issue_id: int
    issue_number: int
    request_id: str
    queue_issue: int
...
@dataclass(frozen=True)
class WeatherV9BrokerPlan:
    authorization: WeatherV9Authorization
...
def _fail(message: str) -> None:
    raise WeatherV9HostCapabilityError(message)
...
def _canonical_json(value: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise WeatherV9HostCapabilityError("authorization cannot be canonicalized") from exc
...
def _parse_utc(value: Any, where: str) -> datetime:
    ...
    try:
        ...
        raise WeatherV9HostCapabilityError(f"{where} is invalid") from exc
...
def parse_authorization_issue(
    ...
    try:
        ...
        raise WeatherV9HostCapabilityError("authorization payload JSON is malformed") from exc
...
def fixed_registry() -> OperationRegistry:
    operation = OperationSpec(
        ...
        dependencies=(
            "source-contract:RPi5_main#601",
            "privileged-delivery:RPi5_main#603",
            "p8-global-mutation-dispatch:disabled",
            "privileged-boundary:identity-only",
            "v7-authority-reuse:forbidden",
        ),
        preflight=("fresh authorization, READY queue, exact current main/CI and v9 predecessor",),
        postconditions=("v9 operator module exact reviewed hash and v8 preserved",),
...
def _public_json(path: str) -> tuple[Mapping[str, Any], datetime]:
    ...
    response = GitHubHttpsSender().send(
        ...
        headers={
            ...
            "User-Agent": "rozkalns-weather-v9-capability/1",
        },
    )
    ...
    try:
        ...
        raise WeatherV9HostCapabilityError("public source response is malformed") from exc
    ...
    try:
        ...
        raise WeatherV9HostCapabilityError("public source Date is invalid") from exc
...
def load_registration() -> Mapping[str, Any]:
    ...
    try:
        ...
        raise WeatherV9HostCapabilityError("host capability registration is malformed") from exc
    required = {
...
def verify_installed_capability(registration: Mapping[str, Any]) -> None:
    targets = (
        (
            SUPPORT_ROOT / "deploy_executor" / "weather_operator_upgrade_v9_host_capability.py",
            0o644,
...
def _require_predecessor() -> None:
    raw = _safe_file(TARGET_PATH, uid=0, gid=0, mode=0o644, max_bytes=2 * 1024 * 1024)
    if hashlib.sha256(raw).hexdigest() != OLD_SHA256:
        _fail("installed predecessor operator module drifted")
...
def prepare_plan(
    ...
    *,
    ...
    try:
        ...
        raise WeatherV9HostCapabilityError("broker request JSON is malformed") from exc
    ...
    if TARGET_TEMP_PATH.exists():
        _fail("fixed v9 operator replacement temp path already exists")
    ...
    state, v9, v8 = _checkout_state(manager, auth.source_sha)
...
def _admit_and_consume(plan: WeatherV9BrokerPlan) -> None:
...
def _bootstrap_checkout(plan: WeatherV9BrokerPlan) -> None:
    _fail("manager-identity worktree bootstrap backend is not installed")


def _run_fixed_upgrade(plan: WeatherV9BrokerPlan) -> Mapping[str, Any]:
    _fail("fixed v9 upgrade backend is not installed")


def execute_plan(plan: WeatherV9BrokerPlan) -> Mapping[str, Any]:
    _admit_and_consume(plan)
    _bootstrap_checkout(plan)
    _run_fixed_upgrade(plan)
    raw = _safe_file(TARGET_PATH, uid=0, gid=0, mode=0o644, max_bytes=2 * 1024 * 1024)
    if hashlib.sha256(raw).hexdigest() != NEW_SHA256:
        _fail("installed v9 operator module postcondition failed")
    return {
        ...
        "v8_checkout_preserved": True,
        "v7_authority_reused": False,
...
def source_readiness() -> Mapping[str, Any]:
    return {
        "schema": "rozkalns.rpi5-main.weather-operator-upgrade-v9-host-capability.v1",
        "issue": ISSUE,
        ...
        "v7_authority_reuse_allowed": False,
