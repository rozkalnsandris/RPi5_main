...
### Phase 4 — Hermes Deals public-repository execution migration — CURRENT: #191 / P9 FRESHNESS REPAIR MERGED / CLEAN REPEAT BLOCKED

Phase 4 remains incomplete. Canonical current continuation is `RPi5_main#191`. Gate B source-App capability proof and Gate C least-privilege D1 credential correction are **PASS / COMPLETE**. Gate D source/host convergence and READY eligibility are proven, and `ops-workflows#27` remains open **READY** as queue eligibility only. Current mutable state must still be fresh-read before every consequential continuation.

READY does not authorize execution. Two owner-authorized P9 incident chains on 2026-08-31 are historical evidence and neither satisfies the P9 exit gate. The first chain is classified by `docs/P9_INCIDENT_EXIT_GATE_RECONCILIATION_2026-08-31.md` and canonical #191 evidence: after a fresh baseline PASS, a malformed `--issue-number #5` shell invocation failed before genuine P9 runtime; a later corrected invocation technically returned `DRY_RUN_READY`, but it was a prohibited retry after the STRICT envelope had already been consumed. That technical result is diagnostic only and cannot retroactively authorize or satisfy the P9 exit gate. A later separately authorized clean-repeat envelope produced one fresh trusted baseline and owner-authored `deploy-authorizations#6`; exactly one correctly formed `rozkalns-deploy-p9 --issue-number 6` invocation then failed closed in `_preflight()` with `P9EvidenceError: Control baseline evidence is stale or from the future`, before `LazyP9StateStore` construction. No retry, cleanup or alternate execution followed. LIVE-AUTH #6 is consumed/non-reusable. No production mutation or P10 is asserted by either incident chain.

`RPi5_main#308` merged the source-only freshness-handoff repair at current-main checkpoint `4a20b48796ef5d09f4bde55fcc3afedcde8920ba`; exact-main Validate #737 / `33435007208`, FAST-LANE #193 / `33435007294`, and GITHUB-ONLY #181 / `33435007209` are successful at that checkpoint. The source keeps baseline freshness at exactly 300 seconds and LIVE-AUTH TTL at exactly 600 seconds, derives a final handoff check from trusted GitHub server time after expensive baseline collection, exposes `observed_at`, `expires_at`, `remaining_freshness_seconds` and a 180-second minimum handoff floor, and refuses publication/PASS when less than 180 seconds remain. Persisted baseline evidence schema is unchanged. The operator sequence now requires all non-live GitHub/source/CI/template preparation before future baseline generation. Source merge does not prove that the trusted host has installed or converged to this repaired CLI.
...
- [x] **P9 EXIT-GATE INCIDENT CLASSIFICATION:** #307 merged the tracked decision `P9_EXIT_GATE=NOT_MET`, `CLEAN_P9_REPEAT_REQUIRED=true`, `P10_BLOCKED=true`; the earlier technical `DRY_RUN_READY` retry is diagnostic only and historical LIVE-AUTH/baselines are not reusable.
- [x] **P9 FRESHNESS-HANDOFF SOURCE REPAIR:** #308 merged the 300-second-preserving freshness diagnostics/handoff guard. It does not assert trusted-host convergence to the repaired CLI.
- [ ] **P9 CLEAN GENUINE READ-ONLY CANARY — CURRENT LIVE STOP:** before any new P9 attempt, merge this Master Plan reconciliation and require fresh exact-main CI. Then separately prove/install trusted-host convergence to the reviewed #308 baseline CLI under an explicit host/runtime authorization if convergence is not already independently proven. Complete all non-live GitHub/source/CI/LIVE-AUTH-template preparation before baseline generation. A new STRICT LIVE envelope must then cover exactly one fresh baseline that reports at least the source-enforced 180-second handoff floor, one new owner-authored 600-second LIVE-AUTH bound to the then-current exact source, and exactly one correctly formed P9 one-shot. Any error/ambiguity after the first authorized LIVE operation means evidence + STOP with no retry/cleanup/alternate path. During genuine P9, no owner trigger, workflow dispatch/rerun, D1 request, deploy or production mutation is allowed.
- [ ] P9 must end with protocol-compliant local `DRY_RUN_READY` and `PRODUCTION_MUTATION_STARTED=false`; P10 remains blocked and separately gated until that clean exit gate is met.
...
### Cross-cutting Track X — owner-authorized pull deploy executor v1 — P8 COMPLETE / P9 FRESHNESS REPAIR MERGED / CLEAN REPEAT BLOCKED
...
Current supersession: canonical `RPi5_main#191` has advanced beyond Gate D READY eligibility into a failed-clean-repeat/freshness-repair state. `RPi5_main#307` merged the tracked incident classification that keeps `P9_EXIT_GATE=NOT_MET`, `CLEAN_P9_REPEAT_REQUIRED=true` and `P10_BLOCKED=true`. Two P9 invocations exist as historical incident evidence, but neither is an accepted P9 exit-gate PASS: the first corrected invocation technically reached `DRY_RUN_READY` only after a prohibited retry inside a consumed STRICT envelope; the later clean-repeat invocation under owner-authored `deploy-authorizations#6` was correctly formed but failed in `_preflight()` because the trusted baseline was stale, before `LazyP9StateStore` construction. No retry followed #6 and no production mutation is asserted. `RPi5_main#308` then merged the source-only freshness-handoff hardening at checkpoint `4a20b48796ef5d09f4bde55fcc3afedcde8920ba`, preserving the 300-second baseline lifetime and 600-second LIVE-AUTH TTL while adding trusted-server-time `observed_at`/`expires_at`/remaining-budget diagnostics and a 180-second minimum handoff floor before publication/PASS. The repaired source is merged and exact-main CI was green at that checkpoint, but source merge is not host convergence and grants no new LIVE authority. The current boundary is therefore **MASTER PLAN RECONCILIATION -> MERGE GATE -> TRUSTED-HOST CONVERGENCE/LIVE STOP -> NEW CLEAN P9**.
...
**#191 / P9 FRESHNESS FOLLOW-UP — MASTER PLAN PR -> MERGE GATE -> TRUSTED-HOST CONVERGENCE/LIVE STOP:** This change reconciles the canonical Master Plan to merged #307/#308 and the consumed #6 stale-baseline incident. It is documentation/source only. Take this focused Master Plan PR through exact-head CI/review and Ready, then STOP for explicit MERGE.

After this reconciliation is merged and exact-main CI is freshly green, re-read `RPi5_main/main`, #191/#236, `ops-workflows#27`, current Control source/CI, the authorization repository and required trusted-host provenance. Do not infer host/runtime convergence from the #308 source merge. If the repaired baseline CLI is not independently proven installed at the exact reviewed source, the next step is a separately explicit STRICT host/runtime convergence authorization before generating another baseline. Complete all non-live preparation before the baseline. No historical READY comment, baseline or LIVE-AUTH (#5/#6 included) is reusable.

A future clean P9 requires a new separately explicit STRICT LIVE owner envelope covering exactly one fresh trusted baseline that satisfies the source-enforced handoff floor, one new owner-authored 600-second LIVE-AUTH bound to the then-current exact source, and exactly one correctly formed P9 one-shot. Any error or ambiguity after the first authorized LIVE operation requires evidence + STOP with no retry/cleanup/rollback/alternate path. Success is only a protocol-compliant local `DRY_RUN_READY` with `PRODUCTION_MUTATION_STARTED=false`. `P9_EXIT_GATE=NOT_MET`, `CLEAN_P9_REPEAT_REQUIRED=true` and `P10_BLOCKED=true` remain binding until that clean exit gate is proven. Phase 6 Hermes Tech work remains deferred while this #191/#236 P9 gate is current.