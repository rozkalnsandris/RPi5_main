...
- Bare `START`, `START RPi5_main`, `SYNC RPi5_main`, `turpini`, or equivalent continuation means normal **FAST-LANE v2.2**. FAST is the safe discovery/audit/non-FULL continuation lane; it is not the default implementation authority and does not activate `AUTO-RUN FULL`.
- For a concrete implementation issue with a usable Definition of Done, the preferred operator lane is the exact explicit form `AUTO-RUN FULL RPi5_main #<issue>`.
...
- Activate `AUTO-RUN FULL` only from the exact explicit form `AUTO-RUN FULL RPi5_main #<issue>` and then read `.github/auto-run-full-v2.json` plus `docs/AUTO_RUN_FULL_V2.md` before any activation write.
...
Read `docs/FAST_LANE_V2_2.md` as the active RPi5-specific FAST contract.

**Primary rule:** the human approves the **RISK / DECISION**; automation executes the **TECHNICAL STEPS**.

- FAST is the safe discovery, audit and non-FULL continuation lane. Use it when scope/risk/DoD still needs to be established or when the owner has not issued an explicit FULL command.
...
- Normal FAST delivery has at most two owner gates: explicit **MERGE**, then one bounded **COMPOSITE LIVE** only when host/runtime mutation is required.
...
Canonical local contract: `.github/auto-run-full-v2.json` and `docs/AUTO_RUN_FULL_V2.md`. Durable controller state: issue `#295`. Roadmap: issue `#315`.

- `AUTO-RUN FULL RPi5_main #<issue>` is the normal implementation lane and one explicit, issue-specific owner decision. It is not blanket repository authority and is never inferred from `START`, `turpini`, prior chat context, issue continuity or controller state.
...
- GitHub event-triggered ChatGPT Work is the preferred low-latency resume path for supported PR activity. The hourly `RPi5 AUTO-RUN` Scheduled Task remains the durable watchdog/fallback; neither path creates new authority.
- The explicit AUTO-RUN FULL command is merge authority only for the canonical PR implementing that exact frozen issue.
- V2 prefers GitHub native auto-merge only after the **final exact current head** has passed fresh diff/scope review, required CI, actionable-review convergence and mergeability checks. Do not arm auto-merge on an unreviewed/mutable head merely to save polling time.
