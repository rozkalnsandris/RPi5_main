...
## Weather public runtime — executable capability source integration

Issue #408 registered the reviewed `rozkalns_weather` public-only handoff as a `STRICT`, non-LIVE-ALL deploy-executor operation with a dedicated execution-disabled adapter and deterministic sanitized baseline resolver contract. Issue #410 added the deterministic first-bootstrap composition. Issue #432 then composed the existing owner LIVE-AUTH/READY/replay protocol into a Weather preactivation envelope, and Issue #435 bound the entire envelope to nine fixed helper identities with whole-preactivation SHA-256 protection. All of those steps remained source-only and host execution stayed disabled. See [the weather executor source contract](WEATHER_PUBLIC_RUNTIME_EXECUTOR_SOURCE.md).

Issue #438 adds the missing executable/installable **source** capability: an immutable `WeatherExecutablePlan`, fixed one-shot helper launcher, root-activation contract, and capability-specific helper implementations for application release, logical volume ensure, schema init, readiness, optional smoke, bounded DWD 10416 truth backfill, bounded ICON-D2/IFS/AIFS forecast backfill, integrity and the 30-minute recurring public-ingest schedule. Generic shell/path/argv/environment authority is not introduced.
