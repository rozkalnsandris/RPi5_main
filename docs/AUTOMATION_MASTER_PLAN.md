...
### Phase 4 — Hermes Deals public-repository execution migration — CURRENT: #191 / P9 GATE C PROVIDER-TOKEN LIVE STOP

Phase 4 remains incomplete. Canonical current continuation is now `RPi5_main#191`, which supersedes the earlier zero-READY / P9-waiting assumption for current routing. A real `ops-workflows#27` now exists with a `[DEPLOY-QUEUE][READY]` title, but its evidence still pins obsolete `RPi5_main` source `3ce0dec33c4191e6703b040f2da9b11e10324ef0`; it is therefore **NOT EXECUTABLE / eligibility drift**, and READY is not authorization. Since that earlier waiting checkpoint, the source-App repair and diagnostics chain advanced through merged `RPi5_main#287` and `#288`, and `RPi5_main#289` merged a bounded fail-closed replacement path for the fixed P9 Control D1 credential. Gate B source-App capability proof is accepted **PASS / COMPLETE**. Gate C is **STOP / NOT GREEN** because no newly owner-created exact-account `Account / D1 / Read` token has an accepted provider-policy receipt and the trusted-RPi5 fixed D1 credential has not been replaced with that proven token. Gate D baseline production and any P9 canary remain downstream and unauthorized.
...
- [ ] **GATE C D1 PROVIDER-SIDE LEAST-PRIVILEGE CORRECTION — CURRENT LIVE STOP:** under a separate explicit owner LIVE authorization, create exactly one new account-owned Cloudflare token for account `70e29dbca0e8363358659102d2b74178` with permission exactly `Account / D1 / Read` and no unrelated/write permission; prove active status and exact account/resource scope, capture only the public-safe token ID/policy metadata, keep the displayed secret owner-local/out-of-band, then STOP.
- [ ] **GATE C HOST CREDENTIAL REPLACEMENT — LATER SEPARATE STRICT LIVE:** only after the provider proof above passes, bind fresh exact merged RPi5 source plus the proven public-safe token ID and execute exactly one reviewed fixed-credential replacement. The reviewed #289 path does not read old credential bytes and has no temp/rename/backup/rollback/retry path. Provider-token creation authorization does not authorize this host transaction.
- [ ] **GATE C RE-PROOF / GATE D BASELINE:** after credential replacement, freshly prove Gate C least privilege/binding. Gate D trusted baseline production remains unauthorized until Gate C is green and then requires its own exact authorization.
...
### Cross-cutting Track X — owner-authorized pull deploy executor v1 — P8 COMPLETE / P9 SOURCE COMPOSITION COMPLETE / GATE C PROVIDER-TOKEN LIVE STOP
...
Current supersession: canonical `RPi5_main#191` has advanced beyond the earlier P9-WAITING checkpoint. `ops-workflows#27` now exists but is **NOT EXECUTABLE / eligibility drift** because its RPi5 source evidence is obsolete. The source-App repair/diagnostic chain merged through `RPi5_main#287` and `#288`; Gate B capability proof is accepted PASS/COMPLETE; and `RPi5_main#289` merged the bounded D1 credential replacement source path. The current boundary is **Gate C provider-token LIVE STOP**: provider-side least privilege must first be proven on a newly owner-created exact-account `Account / D1 / Read` token, followed only later by a separately authorized host credential replacement and fresh Gate C re-proof. Gate D baseline and genuine P9 remain downstream.
...
**#191 / P9 GATE C — PROVIDER TOKEN CREATION + METADATA-SAFE POLICY PROOF LIVE STOP:** the next consequential step requires a separate explicit owner LIVE authorization. Create exactly one new Cloudflare **Account API Token** for exact account `70e29dbca0e8363358659102d2b74178` with permission exactly `Account / D1 / Read`, no D1 Edit/write and no unrelated permission group. Prove active status plus exact account/resource scope and record only the public-safe token ID/status/account/policy evidence. Keep the newly displayed token secret owner-local/out-of-band; never place it in chat, GitHub, argv, env, logs or evidence; then STOP.

That provider-token authorization must not include editing/disabling/deleting legacy `rozkalns-control-setup`, RPi5 checkout or credential mutation, host token verification, D1 query, baseline collection, P9 execution, StateStore, systemd/config/registry, queue/LIVE-AUTH mutation, deploy, cleanup, rollback or retry. After provider proof passes, trusted-RPi5 checkout sync plus exactly one reviewed fixed-credential replacement is a **later separate STRICT LIVE authorization** bound to fresh exact merged source and the proven public-safe token ID. Only after that replacement may Gate C be freshly re-proven; Gate D baseline production remains a separate later authorization.

`ops-workflows#27` is not a current P9 execution candidate despite its READY title because its RPi5 evidence is stale. Do not manufacture a replacement READY or LIVE-AUTH. A future P9 canary still requires fresh reconciled queue/source/CI/baseline/trust evidence, a separate explicit owner decision, and must end only with local `DRY_RUN_READY` plus `PRODUCTION_MUTATION_STARTED=false`. P10 remains a separate live mutation gate. Merge of this documentation reconciliation authorizes none of these LIVE actions. Phase 6 Hermes Tech work remains deferred while this #191/#236 P9 gate is current.
