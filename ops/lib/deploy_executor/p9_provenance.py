...
def _require_platform_guards() -> None:
    for name in ("O_NOFOLLOW", "O_DIRECTORY", "O_CLOEXEC"):
        if type(getattr(os, name, None)) is not int:
            raise ProvenanceError("required no-follow directory guards are unavailable")
    supports_dir_fd = getattr(os, "supports_dir_fd", set())
    if os.open not in supports_dir_fd:
        raise ProvenanceError("required dir_fd open guard is unavailable")
...
def _load_fixed(filename: str, expected_schema: str) -> TrustedEvidence:
    ...
    return TrustedEvidence(payload=payload, sha256=digest, filename=filename)
