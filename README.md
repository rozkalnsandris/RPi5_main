...
V11 tooling is complete and its live-evidence gate is explicit. It attributes the exact AdGuard Home process and container memory to anonymous, file-backed, shared, kernel/socket, slab, and swap classes without reading DNS queries, client data, process arguments, environments, or raw configuration. The supported live workflow collects four spaced samples in one non-root command, verifies every sample and independently verifies the deterministic series summary; issue #27 remains open until that real RPi5 series is collected and interpreted.
