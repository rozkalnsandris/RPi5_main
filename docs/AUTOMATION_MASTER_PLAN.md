...
- [ ] **P9 CANARY OPERATION CONSUMPTION SOURCE GATE — CURRENT:** make only the exact reviewed dormant `hermes-deals.origin-path-audit.v1` operation consumable from the production operation registry while keeping global `execution_enabled=false`. P8 may cease requiring the registry to be literally empty only if it still does not normalize, dispatch, apply or otherwise consume registry operations and remains mutation/result-writer disabled. Source tests must prove the production registry matches the reviewed canary contract, P8 remains read-only with a non-empty but globally disabled registry, adapter preflight remains read-only/disabled and `apply()` still fails closed. No host/runtime wiring or live flag may be enabled by this gate.
...
### Cross-cutting Track X — owner-authorized pull deploy executor v1 — P8 COMPLETE / P9 RUNTIME COMPOSITION COMPLETE / CANARY OPERATION CONSUMPTION SOURCE GATE
...
**#236 / P9 CANARY OPERATION CONSUMPTION SOURCE GATE:** make the exact reviewed dormant `hermes-deals.origin-path-audit.v1` contract consumable from `ops/deploy/executor-operations.json` while keeping the registry globally `execution_enabled=false`. The production registry may contain only that reviewed static canary operation at this gate; its selectors, source identity, baseline resolver, strict authorization class, exclusions, one-invocation budget and adapter identity must match the already-reviewed dormant canary contract.

P8 may stop requiring the production registry to be literally empty only if its runtime still does not normalize, dispatch, apply or otherwise consume operation entries and remains mutation/result-writer disabled. Source tests must prove the production registry matches the reviewed dormant canary contract, the existing P8 poll path remains read-only with a non-empty but globally disabled registry, the Hermes adapter preflight remains `read_only=true`, `execution_enabled=false`, `privileged_dispatch_ready=false`, and `apply()` remains fail-closed. Do not change the installed P8 config/poller deployment/systemd/service/timer, place or rotate credentials, enable privileged dispatch, or change GitHub App permissions/repository selection under this source gate.

Keep `activation_enabled=false`, `runtime_binding_ready=false`, `host_wiring_enabled=false`, `production_mutation_enabled=false` and `PRODUCTION_MUTATION_STARTED=false`. A genuine P9 canary still requires a real READY deploy-queue item, fresh accepted trust/source/baseline evidence and a separate explicit owner decision; do not create dummy READY/LIVE-AUTH artifacts merely to exercise the path. Merge of this or any later source PR remains separately owner-authorized and does not authorize host/runtime/production mutation. Phase 6 Hermes Tech work remains deferred while this #236/P9 gate is current.