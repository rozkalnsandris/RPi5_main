...
def _preflight(expected_sha: str) -> bytes:
    ...
    if os.geteuid() != 0:
        raise UpgradeError("P9 Gate D final-lifecycle producer upgrade requires root")
...
def _write_fd_all(fd: int, data: bytes) -> None:
    ...
    while written < len(view):
        ...
        if count <= 0:
            raise UpgradeError("short write while replacing reviewed Gate D producer")
        written += count
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "P9 Control baseline producer. Source merge alone never authorizes --apply."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.apply:
            print(
                "P9_GATE_D_FINAL_LIFECYCLE_PRODUCER_UPGRADE_PREFLIGHT=PASS "
                f"source_sha={args.expected_sha}"
            )
            print("P9_GATE_D_FINAL_LIFECYCLE_PRODUCER_MUTATION=NO")
            return 0
        ...
        print(f"P9 Gate D final-lifecycle producer upgrade refused: {exc}", file=sys.stderr)
        return 1

    print(
        "P9_GATE_D_FINAL_LIFECYCLE_PRODUCER_UPGRADE=PASS "
        f"source_sha={args.expected_sha}"
    )
