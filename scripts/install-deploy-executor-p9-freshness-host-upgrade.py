...
SCRIPT_RELATIVE = "scripts/install-deploy-executor-p9-freshness-host-upgrade.py"
...
def _reviewed_bytes(expected_sha: str) -> bytes:
    ...
    if _git_blob_sha(reviewed) != TARGET.new_blob_sha:
        raise UpgradeError("reviewed freshness baseline source blob mismatch")
    return reviewed
...
def _preflight(expected_sha: str) -> bytes:
    ...
    if os.geteuid() != 0:
        raise UpgradeError("P9 freshness host upgrade requires root")
...
def _write_fd_all(fd: int, data: bytes) -> None:
    ...
    while written < len(view):
        ...
        if count <= 0:
            raise UpgradeError("short write while replacing reviewed P9 baseline CLI")
        written += count
...
def _replace_exact_target(reviewed_bytes: bytes) -> None:
    ...
    try:
        ...
        if _git_blob_sha(_read_fd_all(fd)) != TARGET.new_blob_sha:
            raise UpgradeError(
                f"installed target post-write blob mismatch: {TARGET.target_path}"
            )
    finally:
        os.close(fd)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "P9 Control baseline CLI with the freshness-handoff repair. "
            "Source merge alone never authorizes --apply."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.apply:
            print(
                "P9_FRESHNESS_HOST_UPGRADE_PREFLIGHT=PASS "
                f"source_sha={args.expected_sha} "
                f"old_blob={TARGET.old_blob_sha} new_blob={TARGET.new_blob_sha}"
            )
            print("P9_FRESHNESS_HOST_MUTATION=NO")
            return 0
        ...
        print(f"P9 freshness host upgrade refused: {exc}", file=sys.stderr)
        return 1

    print(
        "P9_FRESHNESS_HOST_UPGRADE=PASS "
        f"source_sha={args.expected_sha} "
        f"old_blob={TARGET.old_blob_sha} new_blob={TARGET.new_blob_sha}"
    )
