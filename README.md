...
The repository controller runs as the normal operator. Privileged commands are routed to a separately confirmed, versioned and root-owned engine below `/usr/local/libexec/rpi5-deploy/releases/<commit>/`. The root wrapper starts with a clean environment and fixed `PATH`; every root action verifies the installed engine files and requires the current tracked controller/modules/manifest to match the hashes recorded during engine installation.
...
V12 is proposed in Draft PR #31. It adds the root-isolated, human-reviewed, exact-commit deployment engine and transaction workflow for only the three approved non-secret V10 installed files. Production engine installation and target deployment remain separate explicit post-merge actions.
