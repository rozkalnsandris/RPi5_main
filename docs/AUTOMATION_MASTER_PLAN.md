...
The #144 rollback/pre-mutation and health-recovery gate is complete. CV production is reconciled to exact current `f5431265232f356fa27f6204f0cba56e1e730928`, transactional public checks passed, readiness is `CURRENT`, and the recurring CV pull timer remains disabled/inactive. The cross-repository #140 evidence-directory contract and non-deploying host installation/proof are now complete. The first incomplete Phase 3 prerequisite is one separately authorized one-shot `AUTO_DEPLOY_SAFE` execution canary against a genuine newer exact-current-main CV delta; no same-SHA or artificially reclassified target is permitted.
...
**Phase 3 only:** the non-deploying #140 controller/readiness host-install proof is complete. Wait for a genuine newer exact-current-main `rozkalns-cv` delta that deterministically classifies `AUTO_DEPLOY_SAFE`, then prepare one separately reviewed/authorized one-shot controller execution canary. Do not fabricate a same-SHA target, create a dummy runtime change solely to exercise automation, or weaken classification for `NO_DEPLOY`, `MANUAL_ROLLOUT_REQUIRED`, `DB_HOST_APPLY_REQUIRED`, wait or failure states.

For that one-shot canary require fresh exact-current-main push CI, `CONTROL_PLANE_CHANGED=false`, exact installed preflight/classifier/pull-library/pull-wrapper identities, the exact controller↔wrapper evidence contract, identical `READY`/`AUTO_DEPLOY_SAFE` preflight results immediately before mutation, transactional public MIME/cache/nosniff/CSP evidence, final production at the exact target SHA, post-deploy readiness `CURRENT`, timer still disabled/inactive, and unchanged legacy helper/runner. Do not enable recurring execution as part of the canary.
