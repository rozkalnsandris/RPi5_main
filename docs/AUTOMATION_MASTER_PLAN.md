...
- [ ] Install and host-prove the RPi5_main #141 generic-maintenance Compose boundary, then recover `cvbot` through the project-owned CV deploy path and require `/health/ready`/Docker health to return healthy before closing `rozkalns-cv` #144.
- [ ] Complete the `rozkalns-cv` #144 host exit gate after healthy CV recovery and record final health/public/readiness evidence.
...
- [ ] Enable the recurring local controller only after the rollback regression gate and one-shot `AUTO_DEPLOY_SAFE` execution canary are green.
- [ ] Disable/remove the public-repo self-hosted release runner only after the replacement execution path, rollback regression gate and public verification are green.
...
- #144 host exit remained blocked only on runtime health: `cvbot` was `running/unhealthy` and its Docker healthcheck consistently received HTTP 404 from `/health/ready`, while local/public CV HTTP remained reachable;
...
The #140 source boundary must not be installed or exercised on the host until the RPi5_main #141 maintenance policy is installed/proven, CV is recovered through its project-owned deployment path, and the reopened `rozkalns-cv` #144 host exit gate is green with `cvbot` healthy. The CV pull timer remains disabled/inactive throughout those prerequisites.
