...
MODULE = ROOT / "ops/lib/deploy_executor/weather_public_runtime_operator_upgrade_v4.py"
ENTRYPOINT_SOURCE = ROOT / "ops/bin/rozkalns-weather-public-runtime-operator-upgrade-v4"
UPGRADE_CONTRACT = ROOT / "ops/deploy/weather-public-runtime-operator-upgrade-v4.json"
CHECKOUT_CONTRACT = ROOT / "ops/deploy/rpi5-main-weather-public-runtime-operator-upgrade-v4-trusted-checkout-bootstrap.json"
MAKEFILE = ROOT / "Makefile"
sys.path.insert(0, str(ROOT / "ops/lib"))

from deploy_executor import weather_public_runtime_operator_upgrade_v4 as upgrade
...
class WeatherOperatorEntrypointUpgradeV4Tests(unittest.TestCase):
    def test_source_contract_is_fixed_zero_input_and_inactive(self) -> None:
        ready = upgrade.source_readiness()
        self.assertEqual(ready["issue"], 504)
        self.assertEqual(ready["caller_authority"], ())
        self.assertEqual(
            ready["trusted_upgrade_checkout"],
            "RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v4-trusted",
        )
        ...
        self.assertIn(
            '_CHECKOUT_NAME = "RPi5_main-weather-public-runtime-operator-upgrade-v4-trusted"',
            source,
        )
        self.assertIn("execute_upgrade()", source)
        self.assertNotIn("upgrade v3", source)
        for forbidden in (
            "argparse",
            "--path",
            "--command",
    ...
    def test_checkout_contract_is_fixed_and_bounded(self) -> None:
        ...
        target = "RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v4-trusted"
        auth_sha = "EXPLICIT_WEATHER_OPERATOR_UPGRADE_V4_LIVE_EXACT_RPI5_MAIN_SHA"
        ...
        v3 = historical["RPi5_CHECKOUT_PARENT/RPi5_main-weather-public-runtime-operator-upgrade-v3-trusted"]
        self.assertFalse(v3["mutation_allowed"])
        self.assertFalse(v3["cleanup_allowed"])
        self.assertFalse(v3["authority_source"])
    ...
    def test_source_has_atomic_fail_closed_primitives_and_validate_wiring(self) -> None:
        ...
        self.assertIn(
            "python3 ./tests/test-deploy-executor-weather-public-operator-upgrade-v4.py",
            makefile,
        )
