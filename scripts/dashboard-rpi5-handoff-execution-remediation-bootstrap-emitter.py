...
PATH='scripts/dashboard-rpi5-handoff-execution-bundle-remediator.py'
FILE=ROOT/PATH
ACK='RPi5_main:EMIT-DASHBOARD-HANDOFF-EXECUTION-BUNDLE-REMEDIATOR-V1'
MAX=128*1024
...
def main():
    ...
    try:
        st=os.fstat(fd)
        if not stat.S_ISREG(st.st_mode) or st.st_uid!=os.getuid() or st.st_size>MAX: raise Stop('remediator source metadata mismatch')
        data=b''
        while True:
            c=os.read(fd,65536)
            if not c: break
            data+=c
            if len(data)>MAX: raise Stop('remediator source too large')
    finally: os.close(fd)
    if blob(data)!=expected: raise Stop('remediator bytes differ from committed Git blob')
    sys.stdout.buffer.write(data); sys.stdout.buffer.flush()
if __name__=='__main__':
    try: main()
    except Exception as e: print(f'P10_DASHBOARD_HANDOFF_EXEC_REMEDIATION_BOOTSTRAP_EMITTER=STOP reason={type(e).__name__}:{e}',file=sys.stderr); raise SystemExit(1)
