...
Reviewed current official Cloudflare documentation on 2026-08-19.
...
No P1D posture mutation may change client-session authentication or session duration.
...
- exact Control root Access policy preimage only after its P1A exact app exists;
...
- `p1a-08-control-root-exact-app` has completed successfully;
- the exact Control root owner policy exists with no Bypass;
...
Rollback: restore the exact private Control root policy preimage and prove the webhook application is unchanged.
...
Standard Access SSO remains the stable fallback. Current Cloudflare documentation allows bounded Access session durations up to one month, but any convenience change is a separate source decision/canary.
...
- Control depends on P1A exact-root completion;
- client-session Beta remains separate;
